...
/**
 * DKnob is a component similar to JSlider but with round "user interface", a knob.
 * 
...
public class DKnob extends JComponent {
	...
	public void paint(Graphics g) {
		if (isEnabled()) {
			...
			/*
			 * // Draw pointy needle g.drawLine(oOffset + oSizeDiv2, oOffset + oSizeDiv2, x, y); g.setColor(Color.gray);
			 * int s2 = Math.max(size / 6, 6); g.drawOval(oOffset + s2, oOffset + s2, size - s2*2, size - s2*2);
			 * g.drawLine(oOffset + dx + oSizeDiv2, oOffset + dy + oSizeDiv2, x, y); g.drawLine(oOffset - dx +
			 * oSizeDiv2, oOffset - dy + oSizeDiv2, x, y);
			 */
