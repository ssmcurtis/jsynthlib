 ...
 * <p>
 * 
 * Compatibility Note: <code>EnvelopeNode</code> class was replaced by <code>EnvelopWidget.Node</code> class.
 * 
 * @version $Id$
 * @see Node
 * @see SysexWidget
...
public class EnvelopeWidget extends SysexWidget {
	...
	protected void createWidgets() {
		...
		for (int i = 0; i < nodes.length; i++) {
			if (nodes[i].variableX) {
				params[j] = new Param(new JLabel(nodes[i].nameX), new JTextField(
						String.valueOf(nodes[i].pmodelX.get()), 4), i, true);
				params[j].textField.setEditable(false);
				nodes[i].faderNumX = j;
				j++;
			}
			if (nodes[i].variableY) {
				params[j] = new Param(new JLabel(nodes[i].nameY), new JTextField(
						String.valueOf(nodes[i].pmodelY.get()), 4), i, false);
				params[j].textField.setEditable(false);
				nodes[i].faderNumY = j;
				j++;
			}
	...
	public void setSliderNum(int num) {
		_setSliderNum(num);
		setToolTipText("Bank " + ((num - 1) / 16 + 1) + "  Slider " + ((num - 1) % 16 + 1) + "  to  Bank "
				+ ((num + params.length - 2) / 16 + 1) + "  Slider " + ((num + params.length - 2) % 16 + 1));

		for (int j = 0; j < params.length; j++) {
			String t = ("Bank " + ((num - 1) / 16 + 1) + "  Slider " + ((num + j - 1) % 16 + 1));
	...
	/**
	 * Set specified by <code>fader</code> and send System Exclusive message to a MIDI port.
	 * <p>
	 * Called by PatchEditorFrame.faderMoved(byte, byte). This method is used and must be extended by a SysexWidget with
	...
	private final class EnvelopeCanvas extends JPanel {
		...
		private EnvelopeCanvas() {
			...
			for (int i = 0; i < nodes.length; i++) {
				x += nodes[i].maxX;
				if ((nodes[i].maxY + nodes[i].baseY > y) && nodes[i].minY != Node.SAME)
		...
		int xPos(int i, int value) {
			...
			if (i == 0) {
				...
				if (node.invertX)
					return nodes[i - 1].posX + (int) ((node.maxX - value) * scaleX);
				else
		...
		protected void paintComponent(Graphics g) {
			...
			for (int i = 0; i < nodes.length; i++) {
				...
				if ((nodes[i].variableX || nodes[i].variableY) && enabled)
					g.fillRect(nodes[i].posX - DELTA / 2, nodes[i].posY - DELTA / 2, DELTA, DELTA);
				else
					g.drawRect(nodes[i].posX - DELTA / 2, nodes[i].posY - DELTA / 2, DELTA, DELTA);
				if (i > 0)
					g.drawLine(nodes[i].posX, nodes[i].posY, nodes[i - 1].posX, nodes[i - 1].posY);
			}
		}
		...
		private class MyListener extends MouseInputAdapter {
			...
			private boolean isClose(Node node, int x, int y) {
				// ignore static node
				return ((node.variableX || node.variableY) && (Math.abs(x - node.posX) < DELTA) && (Math.abs(y
						- node.posY) < DELTA));
			}
	...
	protected static class Param {
		/**
		 * JLabel widgets which show the names of the X/Y axis parameters riding each access.
		 */
	...
	/**
	 * A data type used by EnvelopeWidget which stores information about a single node (point) in the Widget.
	 * <p>
	 * 
	 * Each <code>Node</code> is one of the movable squares on the envelope. Some of these nodes are stationary, some
	 * contain two parameters on the synth and can be moved vertically and horizontally, and others contain only one
	 * parameter and can therefore be moved in only one direction.
	 * 
	 * @see EnvelopeWidget
	 */
	public static class Node {
		/**
		 * When this value is used for <code>miny</code>, Y axis value remains at whatever the Y axis value of the
		 * previous node was.
		 */
		 ...
		 * <p>
		 * 
		 * Using <code>null</code>s for the Models and Senders and setting min to max means that the node is stationary
		 * on that axis and has no related parameter.
		 * <p>
		 * 
		 * @param minx
		 *            The minimum value permitted by the synth parameter which rides the X axis of the node.
		 * @param maxx
		 *            The maximum value permitted by the synth parameter which rides the X axis of the node.
		 * @param pmodelx
		 *            The Parameter Model which provides reading/writing abilities to the sysex data representing the
		 *            parameter.
		 * 
		 * @param miny
		 *            The minimum value permitted by the synth parameter which rides the Y axis of the node. Using
		 *            <code>Node.SAME</code> for <code>miny</code> means that the height remains at whatever the
		 *            previous node was at.
		 * @param maxy
		 *            The maximum value permitted by the synth parameter which rides the Y axis of the node. When
		 *            <code>Node.SAME</code> is used for <code>miny</code>, this parameter is ignored.
		 * @param pmodely
		 *            The Parameter Model which provides reading/writing abilities to the sysex data representing the
		 *            parameter.
		 * 
		 * @param basey
		 *            The value will be added to all Y values. This doesn't change the function of the EnvelopeWidget,
		 *            but makes it look nicer and possibly be more intuitive to use. Sometimes you don't want zero on a
		 *            Y-axis-riding-parameter to be all the way down at the bottom. This gives it a little bit of rise.
		 * 
		 * @param invertx
		 *            Sometimes on an X-axis-riding attribute 0 is the fastest, other times it is the slowest. This
		 *            allows you to choose.
		 * 
		 * @param senderx
		 *            The ISender which send system exclusive messages to the synths when the Node is moved on the X
		 *            axis direction.
		 * @param sendery
		 *            The ISender which send system exclusive messages to the synths when the Node is moved on the Y
		 *            axis direction.
		 * 
		 * @param namex
		 ...
		 * @see EnvelopeWidget
		 */
		public Node(int minx, int maxx, IParamModel pmodelx, int miny, int maxy, IParamModel pmodely, int basey,
				boolean invertx, ISender senderx, ISender sendery, String namex, String namey) {
			baseY = basey;
			minX = minx;
			maxX = maxx;
			minY = miny;
