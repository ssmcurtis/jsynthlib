...
public class SysexSender implements SysexWidget.ISender {
	/**
	 * Device ID [1-128]. Used by generate(int). Wrong name and should be private. But it's too late, because many
	 * synthdriver's subclasses are using this.
	 */
	protected byte channel;
	private String sysex;
	/**
	 * Creates a new <code>SysexSender</code> instance. A subclass which uses this constructor does not use sysex field.
	 * The subclass has to override <code>generate</code> method.
	 * 
	 * It implies that the subclass does not have to extend this class and can simply implement SysexWidget.ISender.
	 */
	public SysexSender() {
	}

	/**
	 * Creates a new <code>SysexSender</code> instance which uses default <code>generate</code> method.
	 * 
	 * @param sysex
	 *            A <code>String</code> value like <code>"F041##0123**"</code>. Each two letters replesents a hex byte
	 *            data.
	 *            <dl>
	 *            <dt><code>"**"</code>
	 *            <dd>replaced by <code>value</code> passed to the <code>generate</code> method.
	 *            <dt><code>"@@"</code>
	 ...
	 * 
	 * Subclass which uses <code>SysexSender()</code> constructor instead of <code>SysexSender(String)</code> has to
