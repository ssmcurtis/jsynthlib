/**
 * Storage.java - implementation for storing and restoring of objects that
 * support simple persistence (currently implemented as saving to properties
 * files)
 * @version $Id$
...
public class Storage {
	...
	/**
	 * Add the given object to the properties file, prefixing the names of its variables with the given prefix
	 * 
	 * @param storable
	 *            the object to be stored
	 * @param props
	...
	public static void store(Storable storable, Properties props, String prefix) {
		...
		for (Iterator i = propNames.iterator(); i.hasNext();) {
			...
			try {
				PropertyDescriptor prop = new PropertyDescriptor(propName, storable.getClass());
				Method readMethod = prop.getReadMethod();
				Object value = readMethod.invoke(storable, _noArgs);
				storeValue(value, props, propName, prefix);
			} catch (Exception e) {
				ErrorMsg.reportError("Storage Error", "Error storing property '" + propName + "' for class '"
						+ storable.getClass() + "'", e);
			}
		}
	}
	...
	/**
	 * Restore the given object from the properties file, prefixing the names of its variables with the given prefix
	 * 
	 * @param storable
	 *            the object to be restored
	 * @param props
		...
		for (Iterator i = propNames.iterator(); i.hasNext();) {
			...
			try {
				...
				ErrorMsg.reportError("Storage Error",
						"Cannot get property '" + propName + "' for class '" + storable.getClass() + "'", e);
			}
		}
	 ...
	 */
	private static void storeValue(Object value, Properties props, String propName, String prefix) {
		Class propType = value.getClass();

		// Basic types
		if ((propType == Boolean.class) || (propType == Byte.class) || (propType == Character.class)
				|| (propType == Short.class) || (propType == Integer.class) || (propType == Long.class)
				|| (propType == Float.class) || (propType == Double.class) || (propType == String.class)) {
		...
		// Arrays
			...
			props.setProperty(prefix + propName + ".length", String.valueOf(length));
	 ...
	 * @return the value
	 */
	private static Object restoreValue(Class propType, Properties props, String propName, String prefix) {
		...
		// Arrays
			...
			for (int i = 0; i < length; i++) {
				...
				Object value = restoreValue(propType.getComponentType(), props, "[" + i + "]", prefix + propName);
		...
		// Must be a regular object - restore it
			...
			if (className != null) {
				try {
					...
					ErrorMsg.reportError("Config Restore Error", "Error restoring class '" + className + "', " + prefix
							+ propName + ", " + e, e);
				}
			}
			;
