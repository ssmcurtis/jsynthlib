 ...
 * @see SynthDriver
 */
public interface SynthDriverPatch extends SynthDriver {
	/**
	 * Return the size of the patch which the driver handles. <code>SysexGetDialog</code> uses this to estimate timeout
	 * value.
	 * 
	 * @see SysexGetDialog
	 */
	...
	/**
	 * Returns String[] list of patch numbers for writable patches. This can be overridden if some patch locations are
	 * read only. e.g. the Waldorf Pulse has 100 patches, but only 0 to 39 are writable. Currently writable patches are
	 * assumed to start at patch location 0. (This has nothing to with the "Storable" class in JSynthLib.)
	 * 
	 * @see DriverUtil#generateNumbers
	 */
	...
	/**
	 * Check if this driver supports (implements createPatch()) creating a new patch.
	 * 
	 * @see #createPatch()
	 */
	...
	/**
	 * Create a patch from a byte array for the driver. This must be called only when
	 * <code>IDriver.supportsPatch()</code> returns <code>true</code>.
	 * 
	 * @param sysex
	 *            a byte array of sysex data.
	 * @return a array of <code>IPatch</code> object.
	 * @see SynthDriver#supportsPatch(String, byte[])
	...
	/**
	 * Create an array of patches from an array of SysexMessage for the driver. Returns an array of patches because
	 * Converter may be used. This is used for SysexMessages received by using <code>requestPatchDump(int, int)</code>.
	 * 
	 * @param msgs
