 ...
 */
public class MidiTestUtil implements Runnable {
	...
	/**
	 * This runs a few tests on a midi in/out pair. The idea is that you connect the two ports with a midi cable and it
	 * basically tries to send one of every kind of message and it checks to see if those messages get back to the "in"
	 * port intact. - emenake 2003.03.20
	 * 
	 * @param inport
	...
	public static boolean runLoopbackTest(int inport, int outport) {
		ErrorMsgUtil.reportWarning("Before we start", "This process is designed to test the communication of\n"
		...
		try {
			...
			/*
			 * This section is for creating/driving a progress bar. However, since this method is currently getting run
			 * by the UI thread, all UI updates stop while the test is running, so the progress bar never gets updated.
			 * I probably need to execute this thing in a separate thread in order to make this work. - emenaker
			 * 2003.03.26
			 */
			/*
			 * JDialog dialog = new JDialog(); JPanel panel = new JPanel(); JProgressBar pbar = new JProgressBar();
			 * panel.add(pbar); dialog.getContentPane().add(panel); dialog.pack(); dialog.show();
			 * 
			...
			ErrorMsgUtil.reportWarning("Congratulations!", "The test appears to have completed successfully.\n"
					+ "Don't forget to hook your MIDI cables back up to your devices.");
		} catch (Exception e) {
			ErrorMsgUtil.reportError("Warning", "The test failed.\n"
	...
	static void runLoopbackTest(int inport, int outport, MidiMessage msg) throws Exception {
		...
		try {
			...
			if (areEqual(msg, inmsg)) {
				...
				ErrorMsgUtil.reportError("Error",
						"Data Compare Error:" + "\nreceived data: " + MidiUtil.midiMessageToString(inmsg)
								+ "\nexpected data: " + MidiUtil.midiMessageToString(msg));
				throw new Exception("Data mismatch");
			}
		} catch (MidiUtil.TimeoutException e) {
			ErrorMsgUtil.reportError("Warning", "Didn't see anything come into the input");
			throw e;
	...
	/**
	 * This just returns a vector full of MidiMessage objects that the tester uses to send out through the loopback. -
	 * emenaker 2003.03.24
	 * 
	 * @throws Exception
	 */
	...
	/**
	 * This is so that we can run this as a separate thread. I haven't actually done this yet, because I'm worried about
	 * other threads being able to do wierd things with the MidiWrapper while the test is running. - emenaker 2003.03.26
	 */
	public void run() {
	...
	/**
	 * This tests two MidiMessage objects for equality. A byte-by-byte comparison of the actual message data is
	 * performed. - emenaker 2003.03.23
	 * 
	 * @param msg1
	 * @param msg2
	...
	public static boolean areEqual(MidiMessage msg1, MidiMessage msg2) {
		...
		ErrorMsgUtil.reportStatus(msg1 + ", " + msg1size + ": " + msg2 + ", " + msg2size);
