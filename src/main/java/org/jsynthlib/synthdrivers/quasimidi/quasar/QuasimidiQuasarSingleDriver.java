...
public class QuasimidiQuasarSingleDriver extends Driver {
	 ...
	 * 
	 * public int trimSysex(Patch p) { if (p.sysex.length == 644) { byte [] sysex = new
	 * byte[QuasarConstants.PATCH_SIZE];
	 * 
	 * // starting at 19h (=25): Temporary name: 17 Bytes System.arraycopy(p.sysex, 25, sysex,
	 * QuasarConstants.ARRAY_NAME_OFFSET, 17); // starting at 2Ah (= 42): Temporary common parameters: 74 Bytes
	 * System.arraycopy(p.sysex, 42, sysex, 0, 74); // starting at 200h (=512): Part 13: 33 Bytes
	 * System.arraycopy(p.sysex, 512, sysex, QuasarConstants.ARRAY_PART_1_OFFSET, 33); // starting at 221h (=545): Part
	 * 14: 33 Bytes System.arraycopy(p.sysex, 545, sysex, QuasarConstants.ARRAY_PART_2_OFFSET, 33); // starting at 242h
	 * (=578): Part 15: 33 Bytes System.arraycopy(p.sysex, 578, sysex, QuasarConstants.ARRAY_PART_3_OFFSET, 33); //
	 * starting at 263h (=611): Part 16: 33 Bytes System.arraycopy(p.sysex, 611, sysex,
	...
	public static final Patch createNewPatch(int performanceNumber, int sysexOffset) {
		 ...
		 * arp pak 1
		 * 
		 * bit 2 arp_on (0 = off, 1 = on) bits 0-1 arp resolution (00 = 4, 01 = 8, 10 = 16, 11 = 32)
		 */
		 ...
		 * arp pak 2
		 * 
		 * bits 5-6 arp_sync (00 = int, 01 = ext1, 10 = ext2) bits 3-4 arp_dir (00 = up, 01 = down, 10 = up/down) bit 2
		 * arp_sort (0 = off, 1 = on) bit 1 arp_hold (0 = off, 1 = on) bit 0 arp_velo (0 = off, 1 = on)
		 */
		sysex[offset + 22] = (byte) 0x00;
		/*
		 * arp pak 3
		 * 
		 * bits 3-6 arp_track (0000 = 1, 0001 = 2, 0010 = 3, ... , 1111 = 16) bit 2 arp_thru (0 = off, 1 = on) bit 1
		 * arp_out (0 = off, 1 = on) bit 0 arp_freeze (0 = off, 1 = on)
		 */
