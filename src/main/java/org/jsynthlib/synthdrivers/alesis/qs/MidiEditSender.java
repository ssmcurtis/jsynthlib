 ...
 * keyboard. Sends in the format F0 00 00 0E 0E 10 <0mmfffff> <0ssppppp> <0ccccddv> <0vvvvvvvv> F7 (value is 8 bit 2's
 * complement)
 * 
...
public class MidiEditSender extends SysexSender {
	 ...
	 */
	public MidiEditSender(int mode, int function, int sound, int page, int channel, int pot) {
		if (mode < 0 || mode > 3)
	...
	public byte[] generate(int value) {
		if (value < -128 || value > 127)
			throw new IllegalArgumentException("Value must be in the range " + "[-128,127]");
