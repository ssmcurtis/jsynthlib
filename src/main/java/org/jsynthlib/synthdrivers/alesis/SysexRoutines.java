package org.jsynthlib.synthdrivers.alesis;
 ...
 * 
 * <p>
 * Data is in a packed format in order to optimize data transfer. Eight MIDI bytes are used to transmit each block of 7
 * QS data bytes. If the 7 data bytes are looked at as one 56-bit word, the format for transmission is eight 7-bit words
 * beginning with the most significant bit of the first byte, as follows:
 * </p>
 * 
...
public class SysexRoutines {
	...
	/**
	 * Test it out on a real Sysex message that I captured - this is a dump of Mix 90, which has "Jazz Piano" in
	 * position 0, and nothing much in any of the other positions.
	 */
	private static void bigTest() {
		boolean oldTrace = TRACE;
		TRACE = false;
		byte[] bytes = { (byte) , (byte) 0xf7 };
	 ...
	 * @param headerCount
	 *            how many bytes of the sysex are used up as a header - compression starts after the header
	 * @param msBit
	 ...
	 */
	public static int getBits(byte[] sysex, int headerCount, int msBit, int bitSize, boolean signed) {
		int returnVal = 0;
		...
		trace(">>lsBitPos  = " + ((sysex[headerCount + lsByte] & 0xff) >> (lsBitPos)));
		trace("returnVal   = " + returnVal);
	 ...
	 *            the number of bits to return - 1 to 8
	 */
	public static void setBits(int value, byte[] sysex, int headerCount, int msBit, int bitSize) {
		// calculate ending bit
		int lsBit = msBit - bitSize + 1;
		// calculate which byte the bits fall in
		int msByte = msBit / 7;
	 ...
	 */
	public static String getChars(byte[] sysex, int headerCount, int msBit, int count) {
	...
	/**
	 * Given a compressed sysex, set a run of consecutive characters, starting at the given bit. If chars has more
	 * characters than count, set only count characters
	 * 
	 * @param chars
	 ...
	 */
	public static void setChars(String chars, byte[] sysex, int headerCount, int msBit, int count) {
