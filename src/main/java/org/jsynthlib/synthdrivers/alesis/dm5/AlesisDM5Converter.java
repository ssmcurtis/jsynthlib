 ...
 * 
 * When responding to a sysex data dump request, the DM5 sometimes sends a single sysex message and sometimes it sends a
 * block of multiple messages along with garbage data (the behavior is unpredictable). The last valid sysex message
 * message sent is always the one we are interested in but it can be preceded or succeeded with other garbage data. This
 * converter will attempt to parse out the last DM5 patch in the block and discard the rest of the data. It will work
 * with any of the five patch types - system info, edit buffer, program change table, single set, or trigger setup.
 * 
 * @author Jeff Weber
 */
public class AlesisDM5Converter extends ConverterImpl {
	...
	/**
	 * Extracts the last valid patch in a Alesis dump request and validates it. Note that supportsPatch does not convert
	 * the patch. It only attempts to parse out the last patch of a block of data and compare the header to the Alesis
	 ...
	 * headers for each of the DM5 patch types and returns true if there is a match. This is a more thorough check than
	 * the isDM5Patch method, in that it checks the whole header, including the opcode byte.
	 */
	...
	/**
	 * Attempts to parse a Alesis patch (program, edit buffer, or bank) from a Alesis response to a dump request. If
	 * parseSysex cannot find a valid Alesis patch within the block of data, it returns the original block of data,
	 * unchanged.
	 */
	...
	/**
	 * Checks the first 6 bytes of the patch header and returns true if the patch is any of the five DM5 patch types.
	 * This is a generic check that does not include the opcode byte.
	 */
