 ...
 * a single byte of the sysex record contains more than one parameter. Masks are provided for each of the parameters of
...
class BitModel extends ParamModel {
	...
	/**
	 * Constructs a BitModel given the patch, the offset into the sysex record, and the mask representing the parameter.
	 */
	BitModel(PatchDataImpl p, int offset, byte mask) {
		super(p, offset);
		this.mask = mask;
