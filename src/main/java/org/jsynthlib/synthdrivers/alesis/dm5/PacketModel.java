 ...
 * single drumset sysex record. Each "packet" represents all the parameters of a single note within the single drumset
 * sysex record of which there are 60. The parameters for a single note are volume, panning, output, group, drum bank,
 * drum number, fine tune and coarse tune. A single PacketModel can be used to edit one parameter for all 60 notes in
 * the sysex record. The current note being edited is given by packetIndex.
 * 
 * @author Jeff Weber
 */
