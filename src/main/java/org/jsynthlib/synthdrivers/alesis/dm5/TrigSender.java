 ...
 * related to trigger setups--i.e. all the parameters represented by a trigger setup sysex record.
 * 
 * @author Jeff Weber
 */
class TrigSender extends NRPNSender implements SysexWidget.ISender {
	...
	/**
	 * Constructs a TrigSender given the trigger number, the parameter, which represents the mask, and the maximum
	 * value.
	 */
	...
	/**
	 * Sends NRPN messages to the DM5 to adjust the selected parameter. TrigSenders are used exclusively by the
	 * AlesisDM5TrSetEditor to send out NRPNs associated with any of the parameters of the trigger setup record. A
	 * single invocation of the send method will cause two groups of NRPNs to be send. The first group selects the
	 * active trigger and consists of the MSB, the LSB representing the active trigger select command, and the trigger
	...
	public void send(IPatchDriver driver, int value) {
		if (trigNum != LAST_TRIG_NUM) {
			try {
				...
				driver.send(newControlChange(driver, NRPN_LSB, SELECT_ACTIVE_TRG)); // Command to select active trigger
																					// number (NRPN LSB)
