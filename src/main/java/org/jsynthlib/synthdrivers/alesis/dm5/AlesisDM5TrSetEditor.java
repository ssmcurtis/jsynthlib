 ...
 * twelve external triggers on the DM5.
 * 
 * @author Jeff Weber
 */
public class AlesisDM5TrSetEditor extends PatchEditorFrame {
	...
	private void addWidgets(JPanel panel, Patch patch, int trigNum) {
		...
		addWidget(panel, new KnobWidget("Velocy", patch, 0, 7, 0, new ParamModel(patch, 19 + (trigNum * 4)),
				new TrigSender(trigNum, TrigSender.TR_V_CURVE, 7)), 1, 0, 1, 3, fdrNbrBase + 2);

		addWidget(panel, new KnobWidget("X-Talk", patch, 0, 99, 0, new ParamModel(patch, 20 + (trigNum * 4)),
				new TrigSender(trigNum, TrigSender.TR_X_TALK, 99)), 2, 0, 1, 3, fdrNbrBase + 3);

		addWidget(panel, new KnobWidget("Noise Flr", patch, 0, 99, 0, new ParamModel(patch, 21 + (trigNum * 4)),
				new TrigSender(trigNum, TrigSender.TR_NOISE_FLR, 99)), 3, 0, 1, 3, fdrNbrBase + 4);

		addWidget(panel, new KnobWidget("Decay", patch, 0, 99, 0, new ParamModel(patch, 22 + (trigNum * 4)),
				new TrigSender(trigNum, TrigSender.TR_DECAY, 99)), 4, 0, 1, 3, fdrNbrBase + 5);
	}
}