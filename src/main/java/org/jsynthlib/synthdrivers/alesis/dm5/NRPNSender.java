 ...
 * related to single drumsets--i.e. all the parameters represented by a single set sysex record.
 * 
 * @author Jeff Weber
 */
class NRPNSender implements SysexWidget.ISender {
	...
	/**
	 * Constructs an NRPNSender given the parameter, which represents the mask value, and the maximum value.
	 */
	...
	/**
	 * Constructs an NRPNSender given the parameter, which represents the mask, and the maximum value.
	 */
	...
	/**
	 * Sets the number values that the NRPN Sender can send. This is used exclusively by the DM5ComboBoxWidget for the
	 * Drum Sound parameter in the single set editor. When the user selects a new family using the Family ComboBox, the
	 * Drum Sound ComboBox list has to be updated to show the list of voices for the selected family. Since each family
	 * has a different number of voices, the maximum number of voices for the Drum Sound comboBox has to be changed and
	 * the lookup table has to be re- calculated.
	 */
	void setMax(int max) {
		this.max = max;
		this.ccMap = getCCMap(max);
	...
	/**
	 * Recalculates the lookup table used to determine what value to send out when a parameter is changed. Lookup tables
	 * are used instead of calcula- ting the value because the values being sent out generally cover a larger
	 * range--0-127--than the input value. The input value does not have a small enough granularity to accurately
	 * generate the output value using a calculation because of rounding errors and/or dropping of remainders. Since the
	 * DM5 probably uses it's own internal table to convert the values we have to mimic as close as possible what the
	 * DM5 is doing. In some cases this doesn't even work, which is why we furnish another constructor that lets the
	 * user specify the conversion table explicitly.
	 */
	...
	/**
	 * Sends NRPN messages to the DM5 to adjust the selected parameter. NRPNSenders are used exclusively by the
	 * AlesisDM5SgSetEditor to send out NRPNs associated with any of the parameters of the single drumset record. A
