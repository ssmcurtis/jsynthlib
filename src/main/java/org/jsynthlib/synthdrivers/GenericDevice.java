...
public class GenericDevice extends Device {
	...
	private class GenericDriver extends Driver {
		...
		protected JSLFrame editPatch(Patch p) {
			return (new org.jsynthlib.menu.ui.window.HexDumpEditorFrame(p));
		}
	}
	...
	private class IdentityDriver extends Driver {
		...
		protected JSLFrame editPatch(Patch p) {
			int lengthOfID = ManufacturerLookup.lengthOfID(p.sysex, 5);
			String manuf = ManufacturerLookup.get(p.sysex, 5);
