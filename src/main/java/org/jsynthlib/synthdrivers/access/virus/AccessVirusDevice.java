...
public class AccessVirusDevice extends Device /* implements ItemListener */{
	...
	public AccessVirusDevice(Preferences prefs) {
		...
		// Driver drv;
		/*
		 * JOptionPane.showMessageDialog(PatchEdit.getInstance(), DRIVER_INFO, "Access Virus Driver Release Notes",
	...
	/*
	 * public JPanel config() { JPanel panel= new JPanel();
	 * 
	 * panel.add(new JLabel("Select Virus MIDI Device Id")); channelList = new JComboBox(channels);
	 * channelList.setMaximumSize(new Dimension(150, 25)); channelList.setSelectedIndex(deviceId - 1);
	 * channelList.addItemListener(this); panel.add(channelList); return panel; }
	 * 
	 * public void itemStateChanged(ItemEvent e) { if (e.getStateChange() != ItemEvent.SELECTED) { return; } if
	 * (e.getItemSelectable() == channelList) { // avConfig.setDeviceId(channelList.getSelectedIndex() + 1);
	 ...
	 */
	/*
	 * public Set storedProperties() { final String[] storedPropertyNames = { "deviceId" }; Set set =
	 * super.storedProperties(); set.addAll(Arrays.asList(storedPropertyNames)); return set; }
	 */
	/**
