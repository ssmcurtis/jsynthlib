...
public abstract class YamahaMotifSysexUtility {
	...
	public static void checksum(byte[] sysex) {
		for (int size = 0, offset = 0; offset <= sysex.length - SYSEX_OVERHEAD; offset += size + SYSEX_OVERHEAD) {
			...
			checksum(sysex, offset + SIZE_OFFSET, offset + size + SYSEX_OVERHEAD + CHECKSUM_OFFSET);
		}
	}
	...
	public static int findBaseAddressOffset(byte[] sysex, int address) {
		...
		for (int size = 0, offset = 0; offset <= sysex.length - SYSEX_OVERHEAD; offset += size + SYSEX_OVERHEAD) {
			...
			if (sysex[offset + ADDRESS_OFFSET] == high
					...
					&& sysex[offset + ADDRESS_OFFSET + 2] <= low && sysex[offset + ADDRESS_OFFSET + 2] + size >= low)
				return offset;
		}
		return -1;
	}
	...
	public static byte getParameter(byte[] sysex, int address, int offset) {
		if (offset < 0 || offset + ADDRESS_OFFSET + 2 >= sysex.length)
			return -1;
		int index = offset + DATA_OFFSET + (address & 127) - sysex[offset + ADDRESS_OFFSET + 2];
	...
	public static int getShortParameter(byte[] sysex, int address) {
		return getShortParameter(sysex, address, findBaseAddressOffset(sysex, address));
	}
	...
	public static boolean setParameter(byte[] sysex, int address, int value) {
		return setParameter(sysex, address, value, findBaseAddressOffset(sysex, address));
	}
	...
	public static boolean setShortParameter(byte[] sysex, int address, int value) {
		return setShortParameter(sysex, address, value, findBaseAddressOffset(sysex, address));
	}
