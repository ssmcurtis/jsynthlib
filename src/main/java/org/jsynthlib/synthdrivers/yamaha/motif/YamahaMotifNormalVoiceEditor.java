...
public class YamahaMotifNormalVoiceEditor extends PatchEditorFrame {
	...
	protected ComboBoxWidget addCombo(String name, int address, boolean _short, String[] options) {
		return addCombo(name, address, 0, _short, options);
	}

	protected ComboBoxWidget addCombo(String name, int address, int mid, String[] options) {
		return addCombo(name, address, mid, false, options);
	}

	protected ComboBoxWidget addCombo(String name, int address, int mid, boolean _short, String[] options) {
	...
	protected ScrollBarWidget addSlider(String name, int address, int min, int max, int offset) {
		return addSlider(name, address, 0, false, min, max, offset);
	}

	protected ScrollBarWidget addSlider(String name, int address, boolean _short, int min, int max, int offset) {
		return addSlider(name, address, 0, _short, min, max, offset);
	}

	protected ScrollBarWidget addSlider(String name, int address, int mid, int min, int max, int offset) {
		return addSlider(name, address, mid, false, min, max, offset);
	}

	protected ScrollBarWidget addSlider(String name, int address, int mid, boolean _short, int min, int max, int offset) {
		int sendaddress = address;
		if ((byte) (address >> 8) == (byte) -1)
			sendaddress = (address & 0x7f007f) | (mid << 8);
	...
	/*
	 * protected ScrollBarWidget addLabeledSlider(String name, int address, int mid, boolean _short, int min, int max,
	 * int offset) { Box box = Box.createVerticalBox(); panel.add(box); int sendaddress = address; if ( (byte)(address
	 * >> 8) == (byte) -1) sendaddress = (address & 0x7f007f) | (mid << 8); JLabel label = new JLabel(name);
	 * label.setMaximumSize(label.getPreferredSize()); box.add(label); ScrollBarWidget bar = new ScrollBarWidget("", p,
	 * min, max, offset, new MotifParamModel(p, address, mid, _short), new ParamSender(sendaddress, _short)); bar.jlabel
	...
	protected MyScrollBarWidget addLabeledSlider(String name, int address, int mid, boolean _short, int min, int max,
			int offset) {
		int sendaddress = address;
		if ((byte) (address >> 8) == (byte) -1)
			sendaddress = (address & 0x7f007f) | (mid << 8);
	...
	public void addWidget(Container parent, SysexWidget widget, int slidernum, boolean alignleft) {
		try {
			if (parent != null)
				parent.add(widget);
			...
			/*
			 * if (widget instanceof ScrollBarWidget) sliderList.add(((ScrollBarWidget)widget).slider); if (widget
			 * instanceof VertScrollBarWidget) sliderList.add(((VertScrollBarWidget)widget).slider); if (widget
			 * instanceof ScrollBarLookupWidget) sliderList.add(((ScrollBarLookupWidget)widget).slider);
			 */
	...
	class GeneralPanel extends JPanel {
		public GeneralPanel() {
			...
			cat = addCombo("Category", 0x40700C, new String[] { "--", "A. Piano", "Keyboard", "Organ", "Guitar",
					"Bass", "Strings", "Brass", "Reed/Pipe", "Synth Lead", "Syn Pad/Choir", "Synth Comp",
					"Chromatic Perc", "Drum/Perc", "Sound FX", "Musical FX", "Combination" });
			// addHGlue();
			// panel = Box.createHorizontalBox();
			// c.add(panel);
			...
			panel = Box.createHorizontalBox();
			c.add(panel);
			addCombo("Tuning", 0x400004, new String[] { "Equal Temp", "Pure Major C", "Pure Major C#", "Pure Major D",
					"Pure Major D#", "Pure Major E", "Pure Major F", "Pure Major F#", "Pure Major G", "Pure Major G#",
					"Pure Major A", "Pure Major A#", "Pure Major B", "Pure Minor A", "Pure Minor A#", "Pure Minor B",
					"Pure Minor C", "Pure Minor C#", "Pure Minor D", "Pure Minor D#", "Pure Minor E", "Pure Minor F",
					"Pure Minor F#", "Pure Minor G", "Pure Minor G#", "Werkmeister", "Kirnberger", "Vallotti & Young",
					"1/4 Shifted", "1/4 Tone", "1/8 Tone", "Indian" });
			addHGlue();
			addVGlue(c);
			addHGlue(row);
			...
			addCombo("Default Knob Mode", 0x407024, new String[] { "Pan & Send", "Tone", "Assign", "M EQ Offset" });
			// addHGlue();
	...
	class CategoryActionListener implements ActionListener {
		final String[][] subs = new String[][] { { "--" }, { "--", "A. Piano", "E. Grand", "Other" },
				{ "--", "E. Piano", "Clavi", "Other" }, { "--", "Electric", "Pipe", "Other" },
				{ "--", "A. Guitar", "E. Guitar", "Pluck" }, { "--", "A. Bass", "E. Bass", "Synth" },
				{ "--", "Solo", "Ensemble", "Synth" }, { "--", "Solo", "Section", "Synth" },
				{ "--", "Sax", "Pipe", "Other" }, { "--", "Hard", "Soft" }, { "--", "Bright", "Soft", "Choir" },
				{ "--", "Hard", "Soft" }, { "--", "Mallet", "Bell", "Percussion" }, { "--", "Drums", "Percussion" },
				{ "--", "Synth", "Natural" }, { "--", "Motion", "Hit" }, { "--", "Split", "Sequence" } };
		ComboBoxWidget cat, sub;
	...
	class ArpPanel extends JPanel {
		public ArpPanel() {
			...
			addCombo("Unit Multiply", 0x407017, new String[] { "50%", "66%", "75%", "100%", "133%", "150%", "200%" });
			addHGlue();
			panel = Box.createHorizontalBox();
			c.add(panel);
	...
	/*
	 * class EffectsPanel extends JPanel { public EffectsPanel () { super(); setLayout( new BoxLayout(this,
	 * BoxLayout.Y_AXIS) ); } }
	 */
	class ElementPanel extends JPanel {
		...
		public ElementPanel(int i) {
			...
			panel = Box.createHorizontalBox();
			c.add(panel);
			addLabeledSlider("Level Velocity Sensitivity", 0x41ff21, element, false, 0, 0x7f, -64);
	...
	class NumberComboBoxModel implements ComboBoxModel {
		...
		public void setSize(int new_size) {
			...
			if (new_size == size)
				...
				e = new ListDataEvent(this, ListDataEvent.INTERVAL_ADDED, size, new_size - 1);
				while (it.hasNext())
					((ListDataListener) (it.next())).intervalAdded(e);
			} else {
				e = new ListDataEvent(this, ListDataEvent.INTERVAL_REMOVED, new_size, size - 1);
				while (it.hasNext())
