...
public class YamahaUB99BankDumpConverter extends Converter {
	...
	public boolean supportsPatch(String patchString, byte[] sysex) {
		...
		boolean ok = compareString.toString().equalsIgnoreCase(patchString.substring(0, sysexID.length()));
	...
	public Patch[] extractPatch(Patch p) {
		...
		for (int i = 0; i < sm.length; i++) {
			...
			if ((msg[9] == (byte) 0x20) && (msg[10] == (byte) 0x20) && (nr > -1)) {
				System.arraycopy(msg, 13, sysex, YamahaUB99Const.BANK_PATCH_OFFSET + nr * YamahaUB99Const.SINGLE_SIZE,
						32);
				System.arraycopy(msg, 29, sysex, YamahaUB99Const.BANK_NAME_OFFSET + nr * YamahaUB99Const.NAME_SIZE,
