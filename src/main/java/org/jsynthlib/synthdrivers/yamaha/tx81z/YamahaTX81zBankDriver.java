...
public class YamahaTX81zBankDriver extends BankDriver {
	...
	public void putPatch(Patch bank, Patch p, int patchNum) {
		...
		((Patch) bank).sysex[getPatchStart(patchNum) + 6] = (byte) ((((Patch) p).sysex[55] * 64
				+ ((Patch) p).sysex[54] * 8 + ((Patch) p).sysex[56]));// ame ebs kvs
		...
		((Patch) bank).sysex[getPatchStart(patchNum) + 9] = (byte) ((((Patch) p).sysex[53] * 8 + ((Patch) p).sysex[59])); // rs
																																				// dbt
		...
		((Patch) bank).sysex[getPatchStart(patchNum) + 16] = (byte) ((((Patch) p).sysex[55 + 13 * 1] * 64
				+ ((Patch) p).sysex[54 + 13 * 1] * 8 + ((Patch) p).sysex[56 + 13 * 1]));// ame ebs kvs
		...
		((Patch) bank).sysex[getPatchStart(patchNum) + 19] = (byte) ((((Patch) p).sysex[53 + 13 * 1] * 8 + ((Patch) p).sysex[59 + 13 * 1])); // rs
																																									// dbt
		...
		((Patch) bank).sysex[getPatchStart(patchNum) + 26] = (byte) ((((Patch) p).sysex[55 + 13 * 2] * 64
				+ ((Patch) p).sysex[54 + 13 * 2] * 8 + ((Patch) p).sysex[56 + 13 * 2]));// ame ebs kvs
		...
		((Patch) bank).sysex[getPatchStart(patchNum) + 29] = (byte) ((((Patch) p).sysex[53 + 13 * 2] * 8 + ((Patch) p).sysex[59 + 13 * 2])); // rs
																																									// dbt
		...
		((Patch) bank).sysex[getPatchStart(patchNum) + 36] = (byte) ((((Patch) p).sysex[55 + 13 * 3] * 64
				+ ((Patch) p).sysex[54 + 13 * 3] * 8 + ((Patch) p).sysex[56 + 13 * 3]));// ame ebs kvs
		...
		((Patch) bank).sysex[getPatchStart(patchNum) + 39] = (byte) ((((Patch) p).sysex[53 + 13 * 3] * 8 + ((Patch) p).sysex[59 + 13 * 3])); // rs
																																									// dbt

		((Patch) bank).sysex[getPatchStart(patchNum) + 40] = (byte) ((((Patch) p).sysex[105] * 64
				+ ((Patch) p).sysex[100] * 8 + ((Patch) p).sysex[99]));// sync fbl alg
		...
		((Patch) bank).sysex[getPatchStart(patchNum) + 45] = (byte) ((((Patch) p).sysex[107] * 16
				+ ((Patch) p).sysex[108] * 4 + ((Patch) p).sysex[106]));// pms ams lfw
		...
		((Patch) bank).sysex[getPatchStart(patchNum) + 48] = (byte) ((((Patch) p).sysex[117] * 16
				+ ((Patch) p).sysex[110] * 8 + ((Patch) p).sysex[115] * 4 + ((Patch) p).sysex[116]
				* 2 + ((Patch) p).sysex[112]));// ch mo su po pm
		((Patch) bank).sysex[getPatchStart(patchNum) + 49] = (byte) ((((Patch) p).sysex[113])); // porta
																												// time
		...
		((Patch) bank).sysex[getPatchStart(patchNum) + 73] = (byte) ((((Patch) p).sysex[20] * 16
				+ ((Patch) p).sysex[16] * 8 + ((Patch) p).sysex[17]));// egsft,fix,fixrg
		((Patch) bank).sysex[getPatchStart(patchNum) + 74] = (byte) ((((Patch) p).sysex[19] * 16 + ((Patch) p).sysex[18]));// osw
																																				// fine
		((Patch) bank).sysex[getPatchStart(patchNum) + 75] = (byte) ((((Patch) p).sysex[20 + 5] * 16
				+ ((Patch) p).sysex[16 + 5] * 8 + ((Patch) p).sysex[17 + 5]));// egsft,fix,fixrg
		((Patch) bank).sysex[getPatchStart(patchNum) + 76] = (byte) ((((Patch) p).sysex[19 + 5] * 16 + ((Patch) p).sysex[18 + 5]));// osw
																																						// fine
		((Patch) bank).sysex[getPatchStart(patchNum) + 77] = (byte) ((((Patch) p).sysex[20 + 10] * 16
				+ ((Patch) p).sysex[16 + 10] * 8 + ((Patch) p).sysex[17 + 10]));// egsft,fix,fixrg
		((Patch) bank).sysex[getPatchStart(patchNum) + 78] = (byte) ((((Patch) p).sysex[19 + 10] * 16 + ((Patch) p).sysex[18 + 10]));// osw
																																							// fine
		((Patch) bank).sysex[getPatchStart(patchNum) + 79] = (byte) ((((Patch) p).sysex[20 + 15] * 16
				+ ((Patch) p).sysex[16 + 15] * 8 + ((Patch) p).sysex[17 + 15]));// egsft,fix,fixrg
		((Patch) bank).sysex[getPatchStart(patchNum) + 80] = (byte) ((((Patch) p).sysex[19 + 15] * 16 + ((Patch) p).sysex[18 + 15]));// osw
																																							// fine
	...
	public Patch getPatch(Patch bank, int patchNum) {
		try {
			...
			sysex[19] = (byte) ((((Patch) bank).sysex[getPatchStart(patchNum) + 74] & 112) / 16); // Operator
																											// WaveForm
			...
			sysex[24] = (byte) ((((Patch) bank).sysex[getPatchStart(patchNum) + 76] & 112) / 16); // Operator
																											// WaveForm
			...
			sysex[29] = (byte) ((((Patch) bank).sysex[getPatchStart(patchNum) + 78] & 112) / 16); // Operator
																											// WaveForm
			...
			sysex[34] = (byte) ((((Patch) bank).sysex[getPatchStart(patchNum) + 80] & 112) / 16); // Operator
																											// WaveForm
			...
			sysex[66] = (byte) ((((Patch) bank).sysex[getPatchStart(patchNum) + 9 + 10] & 24) / 8);// rate
																											// scaling;
			...
			sysex[79] = (byte) ((((Patch) bank).sysex[getPatchStart(patchNum) + 9 + 20] & 24) / 8);// rate
																											// scaling;
			...
			sysex[92] = (byte) ((((Patch) bank).sysex[getPatchStart(patchNum) + 9 + 30] & 24) / 8);// rate
																											// scaling;
