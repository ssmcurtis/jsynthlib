 ...
 * @version $Id$
 */
public class YamahaTG100SingleDriver extends SynthDriverPatchImpl {
	...
	public YamahaTG100SingleDriver() {
		...
		this.bankNumbers = new String[] { "Internal voice bank" }; // Bank is ignored since there is only one bank for
																	// internal voices
	...
	private void storePatch(PatchDataImpl p, int patchNum) {
		int iTemp = TG100Constants.SYSEX_VOICE_START_ADDRESS3 + (patchNum * TG100Constants.SYSEX_SINGLE_VOICE_SIZE);

		((PatchDataImpl) p).getSysex()[5] = (byte) ((iTemp / 128) + TG100Constants.SYSEX_VOICE_START_ADDRESS2);
		((PatchDataImpl) p).getSysex()[6] = (byte) (iTemp % 128);
	 ...
	 * 
	 * if (sysexRequestDump == null) { JOptionPane.showMessageDialog (PatchEdit.getInstance(), "The " + toString() +
	 * " driver does not support patch getting.\n\n" + "Please start the patch dump manually...", "Get Patch",
	 * JOptionPane.WARNING_MESSAGE); } else { int iTemp = TG100Constants.SYSEX_VOICE_START_ADDRESS3 + (patchNum *
	 * TG100Constants.SYSEX_SINGLE_VOICE_SIZE); int startAddress2 = (int) ((iTemp / 128) +
	 * TG100Constants.SYSEX_VOICE_START_ADDRESS2); int startAddress3 = (int) (iTemp % 128);
	 * 
	 * int sum = 0; int checksum = 0;
	 * 
	 * //sum = 0x4C + 0x4D + 0x20 + 0x20 + 0x30 + 0x30 + 0x36 + 0x38 + 0x52 + 0x51 // + 0x30 + startAddress2 +
	 * startAddress3 + TG100Constants.SYSEX_SINGLE_VOICE_SIZE; //sum = 0x30 + startAddress2 + startAddress3 +
	 * TG100Constants.SYSEX_SINGLE_VOICE_SIZE; sum = 0x30 + startAddress2 + startAddress3;
	 * 
	 * checksum = (byte) (-sum & 0x7f); //checksum = (byte) 0x00;
	 * 
	 * SysexHandler.NameValue[] nameValues = { new SysexHandler.NameValue("adressOffset2", startAddress2 ), new
	 * SysexHandler.NameValue("adressOffset3", startAddress3 ), new SysexHandler.NameValue("byteCount1", 0), new
	 * SysexHandler.NameValue("byteCount2", 0), new SysexHandler.NameValue("byteCount3",
	 * TG100Constants.SYSEX_SINGLE_VOICE_SIZE), new SysexHandler.NameValue("checksum", checksum) };
	 * 
	...
	public PatchDataImpl createNewPatch() {
		PatchDataImpl p = createNewPatch(0);
		calculateChecksum(p);
		return p;
	}
	 ...
	 */
	public static final PatchDataImpl createNewPatch(int patchNum) {
