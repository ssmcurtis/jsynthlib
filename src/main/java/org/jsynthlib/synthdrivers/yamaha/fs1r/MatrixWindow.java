...
class MatrixWindow extends JPanel {
	...
	MatrixWindow(Patch aPatch) {
		...
		for (int i = 0; i < 8; i++) {
			// destinations
			ComboBoxWidget oDest = new ComboBoxWidget("", p, new YamahaFS1RPerformanceDriver.Model(p, 0x40 + i),
					new YamahaFS1RPerformanceDriver.Sender(0x40 + i), YamahaFS1RPerformanceEditor.mDestinations);
			oDest.setLayout(new FlowLayout(FlowLayout.LEFT, 0, 0));
			oTableWidgets[14][i] = oDest;
			// depth
			oTableWidgets[15][i] = new SpinnerWidget("", p, 0, 0x7F, -64, new YamahaFS1RPerformanceDriver.Model(p,
					0x48 + i), new YamahaFS1RPerformanceDriver.Sender(0x48 + i));
			// sources
			for (int s = 0; s < 14; s++) {
				if (s < 7) {
					oTableWidgets[s][i] = new CheckBoxWidget("", p, new YamahaFS1RPerformanceDriver.BitModel(p,
							0x30 + i * 2, 0, 1 << s, s), new YamahaFS1RPerformanceDriver.BitSender(p, 0x30 + i * 2));
				} else {
					oTableWidgets[s][i] = new CheckBoxWidget("", p, new YamahaFS1RPerformanceDriver.BitModel(p,
							0x31 + i * 2, 0, 1 << (s - 7), s - 7), new YamahaFS1RPerformanceDriver.BitSender(p,
							0x31 + i * 2));
				}
			}
			// part switches
			for (int part = 0; part < 4; part++) {
				oTableWidgets[15 + part + 1][i] = new CheckBoxWidget("", p, new YamahaFS1RPerformanceDriver.BitModel(p,
						0x28 + i, 0, 1 << part, part), new YamahaFS1RPerformanceDriver.BitSender(p, 0x28 + i));
			}
		}
	...
	private static class MatrixTableModel extends AbstractTableModel {
		private static final String[] columnNames = { "Mod", "KN1", "KN2", "KN3", "KN4", "MC1", "MC2", "MC3", "MC4",
				"FC", "BC", "MW", "CAT", "PAT", "PB", "Destination", "Depth", "P1", "P2", "P3", "P4" };
