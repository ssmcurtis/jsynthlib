...
class PasteDialog extends JDialog {
	...
	PasteDialog(Frame aOwner, PatchDataImpl aSource, PatchDataImpl aDest, int aPart, ArrayList aWidgets[]) {
		...
		/*
		 * JButton oCopyUnvoices = new JButton ("Paste Unvoices"); oCopyUnvoices.addActionListener (new ActionListener
	...
	private void pasteVoices() {
		...
		int oIndexSource = YamahaFS1RVoiceDriver.VOICE_VOICE_OFFSET + YamahaFS1RVoiceDriver.VOICE_SIZE
		...
		for (int i = 0; i < 8; i++) {
			...
			if (mDest[i].isSelected()) {
				System.arraycopy(mSourcePatch.getByteArray(), oIndexSource, mDestPatch.getByteArray(), oIndexDest,
