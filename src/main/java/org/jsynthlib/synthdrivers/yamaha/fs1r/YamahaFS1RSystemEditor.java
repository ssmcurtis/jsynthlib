...
class YamahaFS1RSystemEditor extends PatchEditorFrame {
	...
	public YamahaFS1RSystemEditor(Patch patch) {
		...
		oMasterPane
				.add(new SpinnerWidget("Note shift", patch, 0, 127, -64, new FS1RModel(patch, 6), new FS1RSender(6)));
		oMasterPane.add(new ComboBoxWidget("Vel curve", patch, new FS1RModel(patch, 0x0E), new FS1RSender(0x0E),
				new String[] { "thru", "soft1", "soft2", "wide", "hard" }));
		oMasterPane.add(new ComboBoxWidget("BC curve", patch, new FS1RModel(patch, 0x0D), new FS1RSender(0x0D),
				new String[] { "thru", "1", "2", "3" }));
		oMasterPane.add(new ComboBoxWidget("KN CtrlMode", patch, new FS1RModel(patch, 0x0B), new FS1RSender(0x0B),
				new String[] { "abs", "rel" }));

		oMasterPane.setBorder(new TitledBorder(LineBorder.createGrayLineBorder(), "Master", TitledBorder.CENTER,
				TitledBorder.CENTER));
