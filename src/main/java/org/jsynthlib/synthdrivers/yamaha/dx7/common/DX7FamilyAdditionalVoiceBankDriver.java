...
public class DX7FamilyAdditionalVoiceBankDriver extends SynthDriverBank {
																		...
																		// needed
	{
		try {
			...
			sysex[6 + 0] = (byte) ((((PatchDataImpl) bank).getSysex()[getPatchStart(patchNum) + 0] & 1)); // OP6 scaling
																											// mode
			// normal/fractional
			// ..(0- 1)
			sysex[6 + 1] = (byte) ((((PatchDataImpl) bank).getSysex()[getPatchStart(patchNum) + 0] & 2) / 2); // OP5
																												// scaling
			// mode
			// normal/fractional
			// ..(0- 1)
			sysex[6 + 2] = (byte) ((((PatchDataImpl) bank).getSysex()[getPatchStart(patchNum) + 0] & 4) / 4); // OP4
																												// scaling
			// mode
			// normal/fractional
			// ..(0- 1)
			sysex[6 + 3] = (byte) ((((PatchDataImpl) bank).getSysex()[getPatchStart(patchNum) + 0] & 8) / 8); // OP3
																												// scaling
			// mode
			// normal/fractional
			// ..(0- 1)
			sysex[6 + 4] = (byte) ((((PatchDataImpl) bank).getSysex()[getPatchStart(patchNum) + 0] & 16) / 16); // OP2
																												// scaling
			// mode
			// normal/fractional
			// ..(0- 1)
			sysex[6 + 5] = (byte) ((((PatchDataImpl) bank).getSysex()[getPatchStart(patchNum) + 0] & 32) / 32); // OP1
																												// scaling
			// mode
			// normal/fractional
			// ..(0- 1)
			sysex[6 + 6] = (byte) ((((PatchDataImpl) bank).getSysex()[getPatchStart(patchNum) + 1] & 7)); // OP6
																											// amplitude
			// modulation
			// sensitivity (0-
			// 7)
			sysex[6 + 7] = (byte) ((((PatchDataImpl) bank).getSysex()[getPatchStart(patchNum) + 1] & 56) / 8); // OP5
																												...
																												// (0-
																												// 7)
			sysex[6 + 8] = (byte) ((((PatchDataImpl) bank).getSysex()[getPatchStart(patchNum) + 2] & 7)); // OP4
																											// amplitude
			// modulation
			// sensitivity (0-
			// 7)
			sysex[6 + 9] = (byte) ((((PatchDataImpl) bank).getSysex()[getPatchStart(patchNum) + 2] & 56) / 8); // OP3
																												...
																												// (0-
																												// 7)
			sysex[6 + 10] = (byte) ((((PatchDataImpl) bank).getSysex()[getPatchStart(patchNum) + 3] & 7)); // OP2
																											// amplitude
																											// modulation
																											// sensitivity
																											// (0- 7)
			...
			// sensitivity
			// (0- 7)
			sysex[6 + 12] = (byte) ((((PatchDataImpl) bank).getSysex()[getPatchStart(patchNum) + 4] & 3)); // Pitch EG
																											// Range
																											// ......................(0-
																											// 3)
			sysex[6 + 13] = (byte) ((((PatchDataImpl) bank).getSysex()[getPatchStart(patchNum) + 4] & 4) / 4); // LFO
																												// key
																												// trigger
																												// (delay)
																												// .............(0-
																												// 1)
			sysex[6 + 14] = (byte) ((((PatchDataImpl) bank).getSysex()[getPatchStart(patchNum) + 4] & 8) / 8); // Pitch
																												// EG by
																												// velocity
																												// switch
																												// .........(0-
			...
			sysex[6 + 15] = (byte) ((((PatchDataImpl) bank).getSysex()[getPatchStart(patchNum) + 5] & 3)); // bit1:Poly/Mono,
																											...
																											// .(0-
																											// 3)
			sysex[6 + 16] = (byte) ((((PatchDataImpl) bank).getSysex()[getPatchStart(patchNum) + 5] & 60) / 4); // Pitch
																												// Bend
			// Range
			// ....................(0-12)
			sysex[6 + 17] = (byte) ((((PatchDataImpl) bank).getSysex()[getPatchStart(patchNum) + 6] & 15)); // Pitch
																											// Bend Step
			// .....................(0-12)
			sysex[6 + 18] = (byte) ((((PatchDataImpl) bank).getSysex()[getPatchStart(patchNum) + 6] & 48) / 16); // Pitch
																													// Bend
			// Mode
			// .....................(0-
			// 3)
			...
			// ..................(0-
			// 7)
			sysex[6 + 20] = (byte) ((((PatchDataImpl) bank).getSysex()[getPatchStart(patchNum) + 7] & 1)); // Portamento
																											// Mode
																											// .....................(0-
																											// 1)
			...
			// Range
			// ......(0-99)
			sysex[6 + 26] = (byte) ((((PatchDataImpl) bank).getSysex()[getPatchStart(patchNum) + 12] & 127)); // Foot
																												// Control
			// 1 Pitch Mod
			// Range
			// ....(0-99)
			sysex[6 + 27] = (byte) ((((PatchDataImpl) bank).getSysex()[getPatchStart(patchNum) + 13] & 127)); // Foot
																												// Control
			// 1 Ampl. Mod
			// Range
			// ....(0-99)
			sysex[6 + 28] = (byte) ((((PatchDataImpl) bank).getSysex()[getPatchStart(patchNum) + 14] & 127)); // Foot
																												// Control
			// 1 EG Bias
			// Range
			// ......(0-99)
			sysex[6 + 29] = (byte) ((((PatchDataImpl) bank).getSysex()[getPatchStart(patchNum) + 15] & 127)); // Foot
																												// Control
			// 1 Volume
			// Range
			// .......(0-99)
			...
			// Bias Range
			// ...(0-100)
			sysex[6 + 34] = (byte) ((((PatchDataImpl) bank).getSysex()[getPatchStart(patchNum) + 20] & 127)); // After
																												// Touch
			// Pitch Mod
			// Range
			// ....(0-99)
			sysex[6 + 35] = (byte) ((((PatchDataImpl) bank).getSysex()[getPatchStart(patchNum) + 21] & 127)); // After
																												// Touch
			// Ampl. Mod
			// Range
			// ....(0-99)
			sysex[6 + 36] = (byte) ((((PatchDataImpl) bank).getSysex()[getPatchStart(patchNum) + 22] & 127)); // After
																												// Touch
			// EG Bias Range
			// ......(0-99)
			sysex[6 + 37] = (byte) ((((PatchDataImpl) bank).getSysex()[getPatchStart(patchNum) + 23] & 127)); // After
																												// Touch
			// Pitch Bias
			// Range
			// ...(0-100)
			sysex[6 + 38] = (byte) ((((PatchDataImpl) bank).getSysex()[getPatchStart(patchNum) + 24] & 7)); // Pitch EG
																											// Rate
			// Scaling
			// ..............(0-
			// 7)
			sysex[6 + 39] = (byte) ((((PatchDataImpl) bank).getSysex()[getPatchStart(patchNum) + 26] & 127)); // Foot
																												// Control
			// 2 Pitch Mod
			// Range
			// ....(0-99)
			sysex[6 + 40] = (byte) ((((PatchDataImpl) bank).getSysex()[getPatchStart(patchNum) + 27] & 127)); // Foot
																												// Control
			// 2 Ampl. Mod
			// Range
			// ....(0-99)
			sysex[6 + 41] = (byte) ((((PatchDataImpl) bank).getSysex()[getPatchStart(patchNum) + 28] & 127)); // Foot
																												// Control
			// 2 EG Bias
			// Range
			// ......(0-99)
			sysex[6 + 42] = (byte) ((((PatchDataImpl) bank).getSysex()[getPatchStart(patchNum) + 29] & 127)); // Foot
																												// Control
			// 2 Volume
			// Range
			// .......(0-99)
			...
			// Volume Range
			// .......(0-99)
			sysex[6 + 47] = (byte) ((((PatchDataImpl) bank).getSysex()[getPatchStart(patchNum) + 34] & 7)); // Unison
																											// detune
			// depth
			// .................(0-
			// 7)
