...
public class YamahaTX802SysexHelpers {
	...
	protected final static SysexHandler Button = new SysexHandler("f0 43 @@ 1B *switch* *OnOff* f7"); // don't care
																										// about "OnOff"
	...
	protected static void chBlock(SynthDriverPatchImpl d, byte ch, byte bn) {
		// port, channel,
		d.send(System.toSysexMessage(ch, new NameValue("param", 0x4d), new NameValue("action", bn)));
	} // bn: 0 = 1-32, 1 = 33-64

	// choose voice mode
	protected static void chVoiceMode(SynthDriverPatchImpl d, byte ch) {// port, channel
		d.send(Button.toSysexMessage(ch, new NameValue("switch", 0x52), new NameValue("OnOff", 0x00)));
