...
class TCBit {
	...
	/**
	 * Constructs a BitModel given the patch, the offset into the m_sysex record, and the m_mask representing the
	 * parameter.
	 */
	...
	/**
	 * Updates the bits defined by m_mask within the byte in the m_sysex record defined by offset with the value
	 * supplied by i.
	 */
	...
	/**
	 * Gets the value of the bits defined by m_mask within the byte in the m_sysex record defined by offset and returns
	 * the value as in int.
	 */
	public int get() {
	...
	/*
	 * String byteToHex(byte i) { return ""+hexDigits[(i & 0xF0) >> 4] +hexDigits[i & 0x0F]; }
	 * 
	 * String printHex() { return byteToHex(m_sysex[m_ofs]) + byteToHex(m_sysex[m_ofs+1]);
	 * 
	 * }
	 * 
	 * String printBin() { return Integer.toBinaryString(m_sysex[m_ofs]) + " " +
