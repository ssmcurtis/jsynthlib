...
public class JV80Constants {
	...
	final static int TONEOFFS[] = { PATCHCOMMONLEN, PATCHCOMMONLEN + PATCHTONELEN, PATCHCOMMONLEN + 2 * PATCHTONELEN,
			PATCHCOMMONLEN + 3 * PATCHTONELEN, };

	final static int PATCHOFFS[] = { 0, PATCHCOMMONLEN, PATCHCOMMONLEN + PATCHTONELEN,
			PATCHCOMMONLEN + 2 * PATCHTONELEN, PATCHCOMMONLEN + 3 * PATCHTONELEN, };
	...
	final static String PATCHNUMBERS[] = DriverUtil.generateNumbers(1, RolandJV80BankDriver.PATCHES_PER_BANK,
			"Patch ##");
	...
	static void calculateChecksum(byte sysex[], int offset, int datalen) {
		DriverUtil.calculateChecksum(sysex, offset + CHECKSUM_START, offset + CHECKSUM_START + datalen + 3, offset
				+ CHECKSUM_START + datalen + 4);
	}
