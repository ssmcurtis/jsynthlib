 ...
 * 
 * Generic SysexWidget.ISender class for Roland DT1 (Data Transfer) system exclusive message.
 * 
...
public class RolandDT1Sender implements SysexWidget.ISender {
	 ...
	 *            Manufacture's ID for System Exclusive Message. Roland : 0x41, 3 byte ID : "00 00 01" -> 0x010000
	 * @param modelId
	 *            Model ID for System Exclusive Message. ("00 3F" -> 0x3f00)
	 * @param addrSize
	 *            Number of byte for address field in System Exclusive Message. 3 byte or 4 byte.
	 * @param addr
	 *            Address offset.
	 * @param dataSize
	 *            Number of byte for data field in System Exclusive Message. 1 byte or 4 byte.
	 * @exception IllegalArgumentException
	 *                if an error occurs
	 */
	public RolandDT1Sender(int manufacturesId, int modelId, int addrSize, int addr, int dataSize)
		...
		b = new byte[5 + (manufacturesId > 255 ? 3 : 1) + (modelId > 255 ? 2 : 1) + addrSize + dataSize];
		int i = 0;
	...
	/**
	 * Constructor without address offset. The address offset must be specified by using setValue(int, int) method in an
	 * overridden setValue(int) method. This is useful for a Sender whose parameter offset address changes dynamically.
	 */
	public RolandDT1Sender(int manufacturesId, int modelId, int addrSize, int dataSize) throws IllegalArgumentException {
	...
	/**
	 * Generate SysEX byte sequence. Address offset must be specified by constructor.
	 * 
	 * @param value
	 *            Data
	 * @exception IllegalArgumentException
