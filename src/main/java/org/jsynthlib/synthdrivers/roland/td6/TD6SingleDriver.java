...
public final class TD6SingleDriver extends Driver {
	...
	/**
	 * Patch must be sent in 13 packets. data packet for the 1st packet is 37 byte, and one for others is 55 byte.
	 */
	private static final int[] PKT_SIZE = { 37, 55, 55, 55, 55, 55, 55, 55, 55, 55, 55, 55, 55 };
	...
	public TD6SingleDriver() {
		...
		/*
		 * for (int i = 1; i <= NUM_PATCH; i++) patchNumbers[i - 1] = (i < 10 ? "0" : "") + String.valueOf(i);
		 */
	 ...
	 * 
	 * TD-6 has 99 drum kits. Drum kit can be selected only by a button on the device. Either Bank select or Program
	 * Change message cannot select a drum kit.
	 * 
	 * A drum kit number is used as address for System Exclusive message. Thus this driver ignores bank number and uses
	 * patch number as Drum Kit number.
	 * 
	...
	/**
	 * Send a Patch (bulk dump system exclusive message) to an edit buffer of MIDI device.
	 * 
	 ...
	 */
	/*
	 * private static byte calcChkSum(byte[] b, int start, int end) { // ErrorMsg.reportStatus("  start = " + start +
	 * ", end = " + end); int sum = 0; for (int i = start; i <= end; i++) sum += b[i]; return (byte) (-sum & 0x7f); }
	 */
	/**
