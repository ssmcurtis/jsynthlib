...
import org.jsynthlib._widgets.VertScrollBarWidget;
...
final class TD6SingleEditor extends PatchEditorFrame {
	...
	TD6SingleEditor(Patch patch) {
		...
		treeWidget = new TreeWidget("Instrument", patch, new Instrument(), new TD6InstModel(patch), new TD6PadSender(
				0x0, true));
		// default
		gbc.gridwidth = 1;
		gbc.gridheight = 1;
		...
		addWidget(topPane, new PatchNameWidget("Drum Kit Name ", patch), 0, 0, 1, 1, 0);
	...
	private JPanel createPadPane(Patch patch) {
		...
		padListPane.setBorder(new TitledBorder(new EtchedBorder(EtchedBorder.RAISED), "Choose a Pad",
		...
		treePane.setBorder(new TitledBorder(new EtchedBorder(EtchedBorder.RAISED), "Instrument Selection",
				TitledBorder.CENTER, TitledBorder.CENTER));
		gbc.gridx = 1;
		gbc.gridy = 0;
		gbc.gridheight = 2;
		...
		padParamPane.setBorder(new TitledBorder(new EtchedBorder(EtchedBorder.RAISED), "Pad Setting",
				TitledBorder.CENTER, TitledBorder.CENTER));
		gbc.gridx = 0;
		gbc.gridy = 1;
		...
		// pitch
		addWidget(padParamPane, new ScrollBarWidget("Pitch", patch, 0, 960, -480, lw,
				new TD6PadModel(patch, 0x4, true), new TD6PadSender(0x4, true)), 0, 0, 1, 1, snum++);
		// decay
		addWidget(padParamPane, new ScrollBarWidget("Decay", patch, 0, 62, -31, lw, new TD6PadModel(patch, 0x8),
				new TD6PadSender(0x8)), 0, 1, 1, 1, snum++);
		addWidget(padParamPane, new ScrollBarLookupWidget("Pad Pattern", patch, 0, 250, lw, new TD6PadModel(patch, 0x9,
				true), new TD6PadSender(0x9, true), padPattern), 0, 2, 1, 1, snum++);
		// Pad Pattern Velocity
		addWidget(padParamPane, new CheckBoxWidget("Pad Pattern Velocity", patch, new TD6PadModel(patch, 0xf),
				new TD6PadSender(0xf)), 0, 3, 1, 1, 0);
		// Pitch Control Switch on/off
		addWidget(padParamPane, new CheckBoxWidget("Pedal Pitch Control", patch, new TD6PadModel(patch, 0x12),
				new TD6PadSender(0x12)), 1, 0, 1, 1, 0);

		lw = (int) (new JLabel("Pedal Pitch Control")).getPreferredSize().getWidth();
		// MIDI gate time (0.1 to 8.0)
		addWidget(padParamPane, new ScrollBarWidget("Gate Time (x 0.1s)", patch, 1, 80, 1, lw, new TD6PadModel(patch,
				0xd), new TD6PadSender(0xd)), 1, 1, 1, 1, snum++);
		// MIDI note number
		addWidget(padParamPane, new ScrollBarWidget("Note Number", patch, 0, 127, 0, lw, new TD6PadModel(patch, 0xe),
				new TD6PadSender(0xe)), 1, 2, 1, 1, snum++);
		return padPane;
	}
	...
	private static final String[] panText = { "L15", "L14", "L13", "L12", "L11", "L10", "L9", "L8", "L7", "L6", "L5",
	...
	private JPanel createMixerPane(Patch patch) {
		...
		padMx.setBorder(new TitledBorder(new EtchedBorder(EtchedBorder.RAISED), "Master", TitledBorder.CENTER,
				TitledBorder.CENTER));
		gbc.gridx = 0;
		gbc.gridy = 0;
		gbc.gridwidth = 1;
		...
		addWidget(padMx, new VertScrollBarWidget(" ", patch, 0, 127, 0, new TD6KitModel(patch, 0x15), new TD6KitSender(
				0x15)), 0, 0, 1, 1, snum++);

		for (int i = 0; i < padList.length; i++) {
			padMx = new JPanel();
			padMx.setLayout(new GridBagLayout());
			padMx.setBorder(new TitledBorder(new EtchedBorder(EtchedBorder.RAISED), padList[i].name,
					TitledBorder.CENTER, TitledBorder.CENTER));
			int w = 0;
			// head
			addWidget(padMx, new VertScrollBarWidget("Head", patch, 0, 127, 0, new TD6KitModel(patch,
					padList[i].offset + 0x00 + 0x10), new TD6KitSender(padList[i].offset + 0x00 + 0x10)), w++, 1, 1, 1,
					snum++);
			// rim
			if (padList[i].dualTrigger && padList[i].dualTriggerActive)
				addWidget(padMx, new VertScrollBarWidget("Head", patch, 0, 127, 0, new TD6KitModel(patch,
						padList[i].offset + 0x13 + 0x10), new TD6KitSender(padList[i].offset + 0x13 + 0x10)), w++, 1,
						1, 1, snum++);

			if (padList[i].name == "Hi-Hat") {
				// Pedal Hi-Hat Volume
				addWidget(padMx, new VertScrollBarWidget("Pedal", patch, 0, 15, 0, new TD6KitModel(patch, 0x13),
						new TD6KitSender(0x13)), w++, 1, 1, 1, snum++);
			}
			// How can I move Center to top? Change DKnob class or add
			// radio button? !!!FIXIT!!!
			addWidget(padMx, new KnobLookupWidget(null, patch, 0, 32, new TD6KitModel(patch, padList[i].offset + 0x26),
					new TD6KitSender(padList[i].offset + 0x26), panText), 0, 0, w, 1, GridBagConstraints.CENTER,
					GridBagConstraints.NONE, snum++);
	...
	private static final String[] studio_type = { "Living Room", "Bathroom", "Recording Studio", "Garage",
	...
	private JPanel createEffectPane(Patch patch) {
		...
		eqPane.setBorder(new TitledBorder(new EtchedBorder(EtchedBorder.RAISED), "Equalizer", TitledBorder.CENTER,
				TitledBorder.CENTER));
		gbc.gridx = 0;
		gbc.gridy = 1;
		...
		ambPane.setBorder(new TitledBorder(new EtchedBorder(EtchedBorder.RAISED), "Ambience", TitledBorder.CENTER,
				TitledBorder.CENTER));
		gbc.gridx = 1;
		gbc.gridy = 1;
		...
		// studio (1 to 9)
		addWidget(eTopPane, new ComboBoxWidget("Studio Type ", patch, 1, new TD6KitModel(patch, 0x8), new TD6KitSender(
				0x8), studio_type), 0, 0, 1, 1, 0);
		// Wall Type
		addWidget(eTopPane, new ComboBoxWidget("Wall Type ", patch, new TD6KitModel(patch, 0xa), new TD6KitSender(0xa),
				wall_type), 1, 0, 1, 1, 0);
		// Room Size (1 to 3)
		addWidget(eTopPane, new ComboBoxWidget("Room Size ", patch, 1, new TD6KitModel(patch, 0xb), new TD6KitSender(
				0xb), room_size), 2, 0, 1, 1, 0);
		// Pedal Pitch Control Range
		addWidget(eTopPane, new ScrollBarWidget("Pedal Pitch Control Range", patch, 0, 48, -24, new TD6KitModel(patch,
				0x14), new TD6KitSender(0x14)), 3, 0, 1, 1, snum++);

		// Master Equalizer Switch
		addWidget(eqPane, new CheckBoxWidget("Enable", patch, new TD6KitModel(patch, 0x11), new TD6KitSender(0x11)), 0,
				0, 1, 1, 0);

		// Equalizer Low Gain
		addWidget(eqPane, new VertScrollBarWidget("Low", patch, 0, 24, -12, new TD6KitModel(patch, 0xd),
				new TD6KitSender(0xd)), 0, 1, 1, 1, snum++);

		// Equalizer High Gain
		addWidget(eqPane, new VertScrollBarWidget("High", patch, 0, 24, -12, new TD6KitModel(patch, 0xf),
				new TD6KitSender(0xf)), 1, 1, 1, 1, snum++);

		// Master Ambience Switch
		addWidget(ambPane, new CheckBoxWidget("Enable", patch, new TD6KitModel(patch, 0x10), new TD6KitSender(0x10)),
				0, 0, 1, 1, 0);

		// Master Ambience Level
		JPanel padEq = new JPanel();
		padEq.setLayout(new GridBagLayout());
		padEq.setBorder(new TitledBorder(new EtchedBorder(EtchedBorder.RAISED), "Master", TitledBorder.CENTER,
				TitledBorder.CENTER));
		gbc.gridx = 0;
		gbc.gridy = 1;
		ambPane.add(padEq, gbc);
		addWidget(padEq, new VertScrollBarWidget(" ", patch, 0, 127, 0, new TD6KitModel(patch, 0x9), new TD6KitSender(
				0x9)), 0, 0, 1, 1, snum++);
		...
		for (int i = 0; i < padList.length; i++) {
			...
			padEq.setLayout(new GridBagLayout());
			// head
			addWidget(padEq, new VertScrollBarWidget("Head", patch, 0, 127, 0, new TD6KitModel(patch,
					(padList[i].offset + 0x00 + 0x11)), new TD6KitSender(padList[i].offset + 0x00 + 0x11)), 0, 1, 1, 1,
					snum++);
			// rim
			if (padList[i].dualTrigger && padList[i].dualTriggerActive)
				addWidget(padEq, new VertScrollBarWidget("Rim", patch, 0, 127, 0, new TD6KitModel(patch,
						(padList[i].offset + 0x13 + 0x11)), new TD6KitSender(padList[i].offset + 0x13 + 0x11)), 1, 1,
						1, 1, snum++);
			padEq.setBorder(new TitledBorder(new EtchedBorder(EtchedBorder.RAISED), padList[i].name,
					TitledBorder.CENTER, TitledBorder.CENTER));
			gbc.gridx = 1 + i;
			gbc.gridy = 1;
			ambPane.add(padEq, gbc);
	...
	final class TD6PadSender extends TD6KitSender {
		...
		protected void setValue(int value) {
			...
			super.setValue(0x01000000 + ((99 - 1) << 16) + padList[pad].offset + (isRim ? 0x13 : 0) + param, value);
			// ErrorMsg.reportStatus("TD6PadSender.setValue: " + this);
		}
	}

	/**
	 * ParamModel of a instruction for selected pad. Used by instrument tree widgets.
	 */
...
class TD6KitSender extends RolandDT1Sender { // extended by TD6PadSender
	...
	TD6KitSender(int drumkit, int param, boolean nibbled) {
		super(0x41, 0x3f00, 4, 0x01000000 + ((drumkit - 1) << 16) + param, nibbled ? 4 : 1);
		this.nibbled = nibbled;
	}

	/**
	 * Creates a new <code>TD6KitSender</code> instance for editing buffer (drum kit 99)
	 * 
	 * @param param
	 *            offset address in kit parameters.
	 * @param nibbled
	...
	/**
	 * Creates a new <code>TD6KitSender</code> instance for editing buffer (drum kit 99) and for 1 byte data.
	 * 
	 * @param param
	 *            offset address in kit parameters.
	 */
	...
	private int conv(int value) {
		return (nibbled ? ((value & 0xf000) << 12 | (value & 0x0f00) << 8 | (value & 0x00f0) << 4 | value & 0x000f)
				: value);
	}
...
class TD6KitModel extends ParamModel { // extended by TD6PadModel
	...
	/**
	 * Convert the relative offset address param to offset address in sysex byte array.
	 * 
	 * @param param
	 *            offset address in kit parameters.
	 * @return an <code>int</code> value
