 ...
 * @version $Id$
 */
public class TD6BankDriver extends SynthDriverBank {
	...
	private static final SysexHandler SYS_REQ = new SysexHandler("F0 41 @@ 00 3F 11 41 7F 00 00 00 00 00 00 40 F7");

	private final TD6SingleDriver singleDriver;

	public TD6BankDriver(TD6SingleDriver singleDriver) {
		super("Bank", "Hiroo Hayashi <hiroo.hayashi@computer.org>", NUM_PATCH, NUM_COLUMNS);
	...
	public String getPatchName(PatchDataImpl p, int patchNum) {
		...
		try {
			return new String(p.getSysex(), nameOfst, patchNameSize, "US-ASCII");
	...
	public void putPatch(PatchDataImpl bank, PatchDataImpl p, int patchNum) {
		System.arraycopy(p.getSysex(), 0, bank.getSysex(), SINGLE_SIZE * patchNum, SINGLE_SIZE);
		for (int i = 0; i < TD6SingleDriver.NUM_PKT; i++) {
			// adjust address field
			bank.getSysex()[getPktOfst(patchNum, i) + 7] = (byte) patchNum;
		}
	...
	public void storePatch(PatchDataImpl p, int bankNum, int patchNum) {
		...
		for (int i = 0; i < NUM_PATCH; i++, ofst += SINGLE_SIZE) {
			singleDriver.storePatch(p.getSysex(), ofst, i);
		}
	}
}
