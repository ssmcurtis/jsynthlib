...
public final class RolandTD6Device extends Device {
	...
	private PadInfo[] padinfo = {
			...
			new PadInfo("Kick", 0x100, false, false, true), new PadInfo("Snare", 0x300, true, false, true),
			new PadInfo("Hi-Hat", 0x700, true, false, true), new PadInfo("Ride", 0xa00, true, true, true),
			new PadInfo("Crash 1", 0x800, true, true, true), new PadInfo("Crash 2", 0x900, true, true, false),
			new PadInfo("Tom 1", 0x400, true, false, true), new PadInfo("Tom 2", 0x500, false, false, true),
			new PadInfo("Tom 3", 0x600, false, false, true), new PadInfo("Tom 4", 0xc00, false, false, false),
	...
	public RolandTD6Device() {
		super("Roland", "TD6", "F07E..0602413F01000000020000f7", "Driver for TD-6 Roland Percussion Sound Module.\n"
	...
	public JPanel config() {
		...
		for (int i = 0; i < padinfo.length; i++) {
			...
			cbox.setSelected(p.getBoolean("dualTriggerActive", padinfo[i].dualTriggerActive));
			cbox.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					JCheckBox cb = (JCheckBox) e.getSource();
	...
	PadInfo[] activePadInfo() {
		...
		for (int i = 0; i < padinfo.length; i++) {
			...
			if (p.getBoolean("padActive", padinfo[i].padActive)) {
				...
				d.padActive = true;
				d.dualTriggerActive = p.getBoolean("dualTriggerActive", padinfo[i].dualTriggerActive);
				activePad[n++] = d;
			}
		}
		return activePad;
	...
	/*
	 * // config variable getter public String getPadName(int i) { return this.padinfo[i].name; }; public boolean
	 * getPadEnableFlag(int i) { return this.padinfo[i].enable; }; public boolean getDualTriggerFlag(int i) { return
	 * this.padinfo[i].dualTrigger; }; // config variable setter public void setPadEnableFlag(int i, boolean val) {
	 * this.padinfo[i].enable = val; }; public void setDualTriggerFlag(int i, boolean val) { this.padinfo[i].dualTrigger
	 * = val; };
	 */
}
