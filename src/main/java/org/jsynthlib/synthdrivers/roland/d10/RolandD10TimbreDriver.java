...
import static org.jsynthlib.synthdrivers.roland.d10.D10Constants.BASE_TIMBRE_TEMP_AREA;
...
public class RolandD10TimbreDriver extends Driver {
	...
	protected String getPatchName(Patch patch) {
		...
		return "Tone " + "abir".substring(toneGroup, toneGroup + 1) + Integer.toString(toneNumber / 8 + 1)
				+ Integer.toString(toneNumber % 8 + 1);
	}

	public void requestPatchDump(int bankNum, int patchNum) {
		D10RequestMessage request = new D10RequestMessage(Entity.createFromIntValue(patchNum).multiply(TIMBRE_SIZE)
				.add(BASE_TIMBRE_MEMORY), TIMBRE_SIZE);
