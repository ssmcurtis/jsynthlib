...
public class RolandD10Support {
	...
	/**
	 * Calculate an address for a memory record expressed in Roland 7 bit hex entity.
	 */
		...
		for (int ab = 0; ab < 2; ab++) {
			for (int bank = 0; bank < 8; bank++) {
				for (int number = 0; number < 8; number++) {
					patchNumbers[ab * 64 + bank * 8 + number] = "AB".substring(ab, ab + 1) + Integer.toString(bank + 1)
	...
	public static void copyPatchData(int patchNum, byte[] bankData, byte[] patchData, boolean fromBank) {
		...
		final int startOffsetInMessage = startLinearOffset - (startsInMessage * MAX_BYTES_PER_MESSAGE);
		final int startOffset = (SYSEX_FRAME_SIZE + MAX_BYTES_PER_MESSAGE) * startsInMessage + SIZE_HEADER_DT1
				+ startOffsetInMessage;
		if (startsInMessage == endsInMessage) {
			...
			if (fromBank) {
				...
				System.arraycopy(bankData, startOffset + firstPartLength + SYSEX_FRAME_SIZE, patchData, SIZE_HEADER_DT1
						+ firstPartLength, secondPartLength);
			} else {
				System.arraycopy(patchData, SIZE_HEADER_DT1, bankData, startOffset, firstPartLength);
				System.arraycopy(patchData, SIZE_HEADER_DT1 + firstPartLength, bankData, startOffset + firstPartLength
