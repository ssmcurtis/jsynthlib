 ...
 * 
 * When responding to a sysex data dump request, the FCB1010 sometimes sends a single sysex message and sometimes it
 * sends a block of multiple messages along with garbage data (the behavior is unpredictable). The last valid sysex
 * message message sent is always the one we are interested in but it can be preceded or succeeded with other garbage
 * data. This converter will attempt to parse out the last FCB patch in the block and discard the rest of the data.
 * 
 * @author Jeff Weber
 */
class FCB1010Converter extends ConverterImpl {
	 ...
	 * patch. It only attempts to parse out the last patch of a block of data and compare the header to the header
	 * bytes. The job of actually converting the patch is handled by extractPatch. If supportsPatch returns true,
	...
	/**
	 * Part of the logic for the overridden supportsPatch method. Checks the the sysex record against the defined header
	 * for the FCB1010 and returns true if there is a match.
	 */
	...
	/**
	 * Attempts to parse an FCB patch from an FCB response to a dump request. If parseSysex cannot find a valid FCB
	...
	/**
	 * Checks the first 6 bytes of the patch header and returns true if the patch is a FCB1010 patch.
	 */
