...
public class CasioCZ1000SingleEditor extends PatchEditorFrame {
	...
	public CasioCZ1000SingleEditor(Patch patch) {
		...
		wideGui = "Wide".equals(patch.getDevice().getPreferences().get("guiMode", null));
	...
	protected JPanel getDca2Panel(Patch patch) {
		...
		if (hackMode)
			addWidget(dca2Panel, new ScrollBarWidget("Key Follow", patch, 0, 0xFFFF, 0, new WordFieldModel(patch,
					CZModel.SAMD, 0xFFFF), null), 0, 0, 7, 1, 0);
		else
			addWidget(dca2Panel, new ScrollBarWidget("Key Follow", patch, 0, 9, 0, new KeyFollowAModel(patch,
					CZModel.SAMD), null), 0, 0, 7, 1, 0);
	...
	protected JPanel getDcw2Panel(Patch patch) {
		...
		if (hackMode)
			addWidget(dcw2Panel, new ScrollBarWidget("Key Follow", patch, 0, 0xFFFF, 0, new WordFieldModel(patch,
					CZModel.SWMD, 0xFFFF), null), 0, 0, 7, 1, 0);
		else
			addWidget(dcw2Panel, new ScrollBarWidget("Key Follow", patch, 0, 9, 0, new KeyFollowWModel(patch,
					CZModel.SWMD), null), 0, 0, 7, 1, 0);
	...
	protected JPanel getDca1Panel(Patch patch) {
		...
		if (hackMode)
			...
			addWidget(dca1Panel, new ScrollBarWidget("Key Follow", patch, 0, 0xFF, 0, new CZModel(patch, TONE_DATA
					+ CZModel.MAMV), null), 0, 0, 7, 1, 0);
		else
			// 10 steps only
			addWidget(dca1Panel, new ScrollBarWidget("Key Follow", patch, 0, 9, 0, new KeyFollowAModel(patch,
					CZModel.MAMD), null), 0, 0, 7, 1, 0);
	...
	protected JPanel getDcw1Panel(Patch patch) {
		...
		if (hackMode)
			// direct access to computed value
			addWidget(dcw1Panel, new ScrollBarWidget("Key Follow", patch, 0, 0xFF, 0, new CZModel(patch, TONE_DATA
					+ CZModel.MWMV), null), 0, 0, 7, 1, 0);
		else
			// 10 steps only
			addWidget(dcw1Panel, new ScrollBarWidget("Key Follow", patch, 0, 9, 0, new KeyFollowWModel(patch,
					CZModel.MWMD), null), 0, 0, 7, 1, 0);
	...
	public JComponent getParamGUI(Patch patch) {
		...
		int row = -1;
		addWidget(miscPane, new ScrollBarLookupWidget("Line Select", patch, 0, 3, 80, new LineModel(patch), null,
				new String[] { "1", "2", "1+1'", "1+2'" }), 0, ++row, 7, 1, 0);

		if (hackMode) {
			addWidget(miscPane, new CheckBoxWidget("Ring", patch, new BitModel(patch, CZModel.MFW + 2, 5), null), 0,
					++row, 1, 1, 0);
			addWidget(miscPane, new CheckBoxWidget("Noise1", patch, new BitModel(patch, CZModel.MFW + 2, 4), null), 1,
					row, 1, 1, 0);
			addWidget(miscPane, new CheckBoxWidget("Noise2", patch, new BitModel(patch, CZModel.MFW + 2, 3), null), 2,
					row, 1, 1, 0);
			// undocumented bit, seems to mute first voice
			addWidget(miscPane, new CheckBoxWidget("Mute1", patch, new BitModel(patch, CZModel.MFW + 2, 2), null), 3,
					row, 1, 1, 0);
		} else {
			addWidget(miscPane, new ScrollBarLookupWidget("Modulation", patch, 0, 3, 80, new ModModel(patch), null,
					new String[] { "OFF", "Ring", "Noise" }), 0, ++row, 7, 1, 0);
		}

		addWidget(miscPane, getDetuneStepGUI(patch), 0, ++row, 7, 1, 0);
		addWidget(miscPane, new ScrollBarWidget("Detune Fine", patch, 0, 63, 0, 80, new DetuneFineModel(patch), null),
				0, ++row, 7, 1, 0);

		// Octave val==3 is same as 1 (+1 octave)
		addWidget(miscPane, new ScrollBarWidget("Octave", patch, -1, 1, 0, 80, new OctaveModel(patch), null), 0, ++row,
				7, 1, 0);
	...
	protected JPanel getVibratoGUI(Patch patch) {
		...
		int row = 0, width = 40;
		/*
		 * --disabled .. weird combos do nothing interesting. addWidget(panel,new CheckBoxWidget("Triangle",patch,new
		 * BitModel(patch,8,3),null),0,row,1,1,0); addWidget(panel,new CheckBoxWidget("Saw Up",patch,new
		 * BitModel(patch,8,2),null),1,row,1,1,0); addWidget(panel,new CheckBoxWidget("Saw Down",patch,new
		 * BitModel(patch,8,5),null),2,row,1,1,0); addWidget(panel,new CheckBoxWidget("Square",patch,new
		 * BitModel(patch,8,1),null),3,row,1,1,0);
		 */
		addWidget(panel, new ScrollBarLookupWidget("Wave", patch, 0, 4, width, new VibratoWaveModel(patch), null,
				new String[] { "OFF", "1 Tri", "2 Up", "3 Dn", "4 Pul" }), 0, row, 7, 1, 0);

		if (hackMode) {
			// Rate is limited by the computed 16-bit value
			addWidget(panel, new ScrollBarWidget("Delay", patch, 0, 255, 0, width, new VibratoDelayModel(patch), null),
					0, ++row, 7, 1, 0);
			addWidget(panel, new ScrollBarWidget("Rate", patch, 0, 142, 0, width, new VibratoRateModel(patch), null),
					0, ++row, 7, 1, 0);
			addWidget(panel, new ScrollBarWidget("Depth", patch, 0, 255, 0, width, new VibratoDepthModel(patch), null),
					0, ++row, 7, 1, 0);
		} else {
			// Officially limited to 0-99.
			addWidget(panel, new ScrollBarWidget("Delay", patch, 0, 99, 0, width, new VibratoDelayModel(patch), null),
					0, ++row, 7, 1, 0);
			addWidget(panel, new ScrollBarWidget("Rate", patch, 0, 99, 0, width, new VibratoRateModel(patch), null), 0,
					++row, 7, 1, 0);
			addWidget(panel, new ScrollBarWidget("Depth", patch, 0, 99, 0, width, new VibratoDepthModel(patch), null),
					0, ++row, 7, 1, 0);
		}
		return panel;
	}
	...
	protected JPanel getWaveformGUI(Patch patch, int offset) {
		...
		String[] wfLabels = new String[] { "1 Saw", "2 Pul", "3 Tri", "Null", "4 TrSi", "5 SiPu", "SinSy", "NPul"
		...
		// Labeled sliders
		addWidget(panel, new ScrollBarLookupWidget("Waveform 1", patch, 0, wfLabels.length - 1, 80, new WordFieldModel(
				patch, TONE_DATA + offset, 0xE000), null, wfLabels), 0, ++row, 7, 1, 0);
		addWidget(panel, new ScrollBarLookupWidget("Waveform 2", patch, 0, wfLabels.length - 1, 80, new WordFieldModel(
				patch, TONE_DATA + offset, 0x1C00), null, wfLabels), 0, ++row, 7, 1, 0);
		addWidget(panel, new CheckBoxWidget("Enable WF 2", patch, new BitModel(patch, offset, 1), null), 0, ++row, 1,
				1, 0);
		addWidget(panel, new ScrollBarLookupWidget("Window", patch, 0, resLabels.length - 1, 80, new WordFieldModel(
				patch, TONE_DATA + offset, 0x01C0), null, resLabels), 0, ++row, 7, 1, 0);
		return panel;
	}

	/**
	 * "Normal mode": envelope control give access to params as seen on LCD. Uses sliders for everything.
	 * 
	 * @param patch
	 *            work area
	 * @param firstOffset
	...
	protected JPanel getEnvelopeGUI1(Patch patch, int firstOffset) {
		...
		JPanel panel = new JPanel();
		panel.setLayout(new GridBagLayout());
		panel.setBorder(new TitledBorder(new EtchedBorder(EtchedBorder.RAISED), "Envelope", TitledBorder.LEFT,
				TitledBorder.CENTER));
		...
		if (hackMode)
			...
			maxval = 99;
		addWidget(panel, new ScrollBarWidget("1 Rate", patch, 0, maxval, 0, width, model.getRateModel(0), null), 0,
				++row, 1, 1, 0);
		addWidget(panel, new ScrollBarWidget("Level", patch, 0, maxval, 0, width, model.getLevelModel(0), null), 1,
				row, 1, 1, 0);
		addWidget(panel, new CheckBoxWidget("Sustain", patch, model.getSustainModel2(0), null), 2, row, 1, 1, 0);
		addWidget(panel, new ScrollBarWidget("2 Rate", patch, 0, maxval, 0, width, model.getRateModel(1), null), 0,
				++row, 1, 1, 0);
		addWidget(panel, new ScrollBarWidget("Level", patch, 0, maxval, 0, width, model.getLevelModel(1), null), 1,
				row, 1, 1, 0);
		addWidget(panel, new CheckBoxWidget("Sustain", patch, model.getSustainModel2(1), null), 2, row, 1, 1, 0);
		addWidget(panel, new ScrollBarWidget("3 Rate", patch, 0, maxval, 0, width, model.getRateModel(2), null), 0,
				++row, 1, 1, 0);
		addWidget(panel, new ScrollBarWidget("Level", patch, 0, maxval, 0, width, model.getLevelModel(2), null), 1,
				row, 1, 1, 0);
		addWidget(panel, new CheckBoxWidget("Sustain", patch, model.getSustainModel2(2), null), 2, row, 1, 1, 0);
		addWidget(panel, new ScrollBarWidget("4 Rate", patch, 0, maxval, 0, width, model.getRateModel(3), null), 0,
				++row, 1, 1, 0);
		addWidget(panel, new ScrollBarWidget("Level", patch, 0, maxval, 0, width, model.getLevelModel(3), null), 1,
				row, 1, 1, 0);
		addWidget(panel, new CheckBoxWidget("Sustain", patch, model.getSustainModel2(3), null), 2, row, 1, 1, 0);
		addWidget(panel, new ScrollBarWidget("5 Rate", patch, 0, maxval, 0, width, model.getRateModel(4), null), 0,
				++row, 1, 1, 0);
		addWidget(panel, new ScrollBarWidget("Level", patch, 0, maxval, 0, width, model.getLevelModel(4), null), 1,
				row, 1, 1, 0);
		addWidget(panel, new CheckBoxWidget("Sustain", patch, model.getSustainModel2(4), null), 2, row, 1, 1, 0);
		addWidget(panel, new ScrollBarWidget("6 Rate", patch, 0, maxval, 0, width, model.getRateModel(5), null), 0,
				++row, 1, 1, 0);
		addWidget(panel, new ScrollBarWidget("Level", patch, 0, maxval, 0, width, model.getLevelModel(5), null), 1,
				row, 1, 1, 0);
		addWidget(panel, new CheckBoxWidget("Sustain", patch, model.getSustainModel2(5), null), 2, row, 1, 1, 0);
		addWidget(panel, new ScrollBarWidget("7 Rate", patch, 0, maxval, 0, width, model.getRateModel(6), null), 0,
				++row, 1, 1, 0);
		addWidget(panel, new ScrollBarWidget("Level", patch, 0, maxval, 0, width, model.getLevelModel(6), null), 1,
				row, 1, 1, 0);
		addWidget(panel, new CheckBoxWidget("Sustain", patch, model.getSustainModel2(6), null), 2, row, 1, 1, 0);
		addWidget(panel, new ScrollBarWidget("8 Rate", patch, 0, maxval, 0, width, model.getRateModel(7), null), 0,
				++row, 1, 1, 0);
		addWidget(panel, new ScrollBarWidget("Level", patch, 0, maxval, 0, width, model.getLevelModel(7), null), 1,
				row, 1, 1, 0);
		addWidget(panel, new CheckBoxWidget("Sustain", patch, model.getSustainModel2(7), null), 2, row, 1, 1, 0);
		addWidget(panel, new ScrollBarWidget("Final Stage", patch, 0, 7, 1, model.getEndStageModel(), null), 0, ++row,
				1, 1, 0);
		return panel;
	}

	/**
	 * "Normal mode": envelope control give access to params as seen on LCD. This is an experimental panel that uses the
	 * EnvelopeWidget. Doesn't work for some reason.
	 * 
	 * @param patch
	 *            work area
	 * @param firstOffset
	...
	protected JPanel getEnvelopeGUI2(Patch patch, int firstOffset) {
		...
		EnvelopeWidget.Node[] nodes = new EnvelopeWidget.Node[] {
				...
				new EnvelopeWidget.Node(0, maxval, model.getRateModel(0), 0, maxval, model.getLevelModel(0), 0, false,
						new NullSender(), new NullSender(), "1 Rate", "   Level"),
				new EnvelopeWidget.Node(0, maxval, model.getRateModel(1), 0, maxval, model.getLevelModel(1), 0, false,
						new NullSender(), new NullSender(), "2 Rate", "   Level"),
				new EnvelopeWidget.Node(0, maxval, model.getRateModel(2), 0, maxval, model.getLevelModel(2), 0, false,
						new NullSender(), new NullSender(), "3 Rate", "   Level"),
				new EnvelopeWidget.Node(0, maxval, model.getRateModel(3), 0, maxval, model.getLevelModel(3), 0, false,
						new NullSender(), new NullSender(), "4 Rate", "   Level"),
				new EnvelopeWidget.Node(0, maxval, model.getRateModel(4), 0, maxval, model.getLevelModel(4), 0, false,
						new NullSender(), new NullSender(), "5 Rate", "   Level"),
				new EnvelopeWidget.Node(0, maxval, model.getRateModel(5), 0, maxval, model.getLevelModel(5), 0, false,
						new NullSender(), new NullSender(), "6 Rate", "   Level"),
				new EnvelopeWidget.Node(0, maxval, model.getRateModel(6), 0, maxval, model.getLevelModel(6), 0, false,
						new NullSender(), new NullSender(), "7 Rate", "   Level"),
				new EnvelopeWidget.Node(0, maxval, model.getRateModel(7), 0, maxval, model.getLevelModel(7), 0, false,
						new NullSender(), new NullSender(), "8 Rate", "   Level"), };
		...
		// addWidget(panel, new ScrollBarWidget("Sustain Stage", patch, -1, 7, 1, 70, model.getSustainModel(), null), 0,
		// ++row, 1, 1, 0);
		addWidget(panel, new ScrollBarLookupWidget("Sustain Stage", patch, -1, 7, 70, model.getSustainModel(), null,
				new String[] { "OFF", "1", "2", "3", "4", "5", "6", "7", "8" }), 0, ++row, 1, 1, 0);
		addWidget(panel, new ScrollBarWidget("Final Stage", patch, 0, 7, 1, 70, model.getEndStageModel(), null), 0,
				++row, 1, 1, 0);
		return panel;
	}
	...
	/**
	 * Vibrato params have this weird multi-segment curve, and are stored in three bytes: first byte is the raw value.
	 * Second and third are the low and high bytes from the appropriate point on the curve.
	 */
	...
	/**
	 * Master controller for envelope params. Create one of these for an envelope, then use get*Model to get the objects
	 * to feed to the widgets.
	 */
	class EnvelopeModel extends CZModel {
		...
		public void setSustainStage(int stage) {
			if (stage < -1 || stage > 7)
				throw new IllegalArgumentException("stage=" + stage + "; must be -1 or 0..7");
			for (int i = 0; i < 8; i++) {
				int o = ofs + 4 + 4 * i;
		...
		public void setSustainStage(int stage, int value) {
			if (stage < 0 || stage > 7)
				throw new IllegalArgumentException("stage=" + stage + "; must be 0..7");
			int o = ofs + 4 + 4 * stage;
			if (value == 0)
				setByte(o, getByte(o) & 0x7F);
			else
