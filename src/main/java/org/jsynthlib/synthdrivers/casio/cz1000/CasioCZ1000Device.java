 ...
 * Casio itself), you will get a patch of 263 bytes. you CAN'T send that patch back to the Casio... The patch must be
...
public class CasioCZ1000Device extends Device {
	private static final String INFO_TEXT = "Casio CZ-101/1000."
			...
			+ "  No work done at all for the CZ-5000, though it's" + " fundamentally the same."
			+ "\n    Note that Casio doesn't support 'senders':  the sliders"
			+ " in the editor don't update the synth live.  You'll need to"
			+ " double-click the patch in the library to send the whole thing" + " over."
			+ "\n    There are two GUI modes:  \"Stacked\" keeps a small, tight"
			+ " window by using nested tabs.  \"Wide\" keeps more parameters visible" + " at once."
			+ "\n    There are also two parameter modes:  \"Extended\""
			+ " reveals full access to all sysex bits and bytes." + "  \"Normal\" restricts GUI to official limits.";

	static final String[] GUI_OPTIONS = new String[] { "Stacked", "Wide" };
	static final String[] PARAM_OPTIONS = new String[] { "Normal", "Extended" };

	public CasioCZ1000Device() {
		super("Casio", "CZ-101/1000", null, INFO_TEXT, "Yves Lefebvre, Bill Zwicky");
	}
	...
	public JPanel config() {
		...
		final ComboBoxWidget guiMode = new ComboBoxWidget("GUI Style", null, null, null, GUI_OPTIONS);
		...
		ComboBoxWidget paramMode = new ComboBoxWidget("Parameter Set", null, null, null, PARAM_OPTIONS);
