 ...
 * 
 * When responding to a sysex data dump request, the PODs sometimes send a single sysex message and sometimes they send
 * a block of multiple messages along with garbage data (the behavior is unpredictable). The last valid sysex message
 * message sent is always the one we are interested in but it can be preceded or succeeded with other garbage data. This
 * converter will attempt to parse out the last Pod patch in the block and discard the rest of the data. It will work
 * with any of the three patch types (program, edit buffer, or bank). This converter will also convert an edit buffer
 * patch to a normal program patch.
 * 
 * @author Jeff Weber
 */
public class Line6BassPodConverter extends ConverterImpl {
	...
	/**
	 * Extracts the last valid patch in a Line6 dump request and validates it. Note that supportsPatch does not convert
	 * the patch. It only attempts to parse out the last patch of a block of data and compare the header to the Line6
	...
	public boolean supportsPatch(String patchString, byte[] sysex) {
		if (super.supportsPatch(patchString, parseSysex(sysex))) { // Parse out the last patch in the block and test it.
			if ((sysex.length == Constants.PDMP_HDR_SIZE + Constants.SIGL_SIZE + 1)
					|| (sysex.length == Constants.BDMP_HDR_SIZE + (Constants.SIGL_SIZE * Constants.PATCHES_PER_BANK)
							+ 1)) {
				return false; // If sysex has already been converted to program or bank, Converter no longer supports
								// it.
			} else {
	...
	public PatchDataImpl[] extractPatch(PatchDataImpl p) {
		...
		if (sysex[6] == (byte) 0x00) { // Is this a program patch?
			newPatchArray[0] = new PatchDataImpl(sysex, new Line6BassPodSingleDriver());
		} else {
			newPatchArray[0] = new PatchDataImpl(sysex, new Line6BassPodBankDriver());
		}
		return newPatchArray;
	}

	/**
	 * Attempts to parse a Line6 patch (program, edit buffer, or bank) from a Line6 response to a dump request. If
	 * parseSysex cannot find a valid Line6 patch within the block of data, it returns the original block of data,
	 * unchanged.
	 */
				...
				if ((i + 6) < sysex.length) {
					if ((sysex[i + 1] == Constants.SIGL_DUMP_HDR_BYTES[1])
						...
						if (sysex[i + 6] == Constants.BANK_DUMP_HDR_BYTES[6]) {
							patchLength = Constants.BDMP_HDR_SIZE + (Constants.SIGL_SIZE * Constants.PATCHES_PER_BANK)
									+ 1;
						}
	...
	/**
	 * Converts a Line6 Edit Buffer patch to a Line6 program patch. Any time an edit buffer patch is received from a
	 * Line6 device it is converted to a program patch. From that point on it is handled like any other program patch.
	 */
	private byte[] convertToProgramPatch(byte[] sysex) {
		...
		System.arraycopy(sysex, Constants.EDMP_HDR_SIZE, newSysex, Constants.PDMP_HDR_SIZE, newSysexLength
