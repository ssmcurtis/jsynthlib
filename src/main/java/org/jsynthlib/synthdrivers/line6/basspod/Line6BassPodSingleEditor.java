...
public class Line6BassPodSingleEditor extends PatchEditorFrame {
	 ...
	 */
	static final String[] ampModel = { "Tube Preamp", "Session", "California", "Jazz Tone", "Adam and Eve", "Eighties",
			"Stadium", "Amp 360", "Rock Classic", "Brit Major", "Brit Super", "Silver Panel", "Brit Class A",
			"Motor City", "Flip Top", "Sub Dub" };

	/**
	 * Array of strings representing the descriptions of all amp models for the Line6 Pod.
	 */
	static final String[] ampModelDesc = { "Tube Preamp", "SWR SM-400", "Mesa Boogie Bass 400+", "Polytone Mini-Brute",
			"Eden Traveller WT-300", "Gallien Kruger 800RB", "Sunn Colliseum", "Acoustic 360", "Ampeg SVT",
			"Marshall Major", "Marshall Super Bass", "Fender Bassman Head", "Vox AC-100", "Versatone Pan-O-Flex",
	 ...
	 */
	static final String[] cabType = { "8x10 Ampeg SVT", "4x10 Eden", "4x10 SWR Goliath", "4x10 Hartke",
			"1x12 Versatone Pan-O-Flex", "4x12 Marshall 1967 w/Celestions", "1x15 Ampeg B-15",
			"1x15 Polytone Mini-Brute", "2x15 Vox AC-100", "2x15 Mesa Boogie", "2x15 Fender Bassman w/JBLSs",
			"4x15 1976 Marshall", "1x18 SWR Big Ben", "1x18 Acoustic 360", "1x12-1x18 Sunn Coliseum 8028",
			"No Cabinet Emulation" };

	/**
	 * Array of strings representing the descriptions of all effect models for the Line6 Pod.
	 */
	static final String[] effectType = { "Octave Down (Boss OC-2)", "Analog Chorus (Boss CE-1)",
			"Danish Chorus (t.c. Stereo Chorus)", "Orange Phase (Phase 90)", "Gray Flanger (MXR Flanger)",
			"Tron Down (Mu-Tron III)", "Tron Up (Mu-Tron III)", "Sample & Hold (Oberheim VCF)", "S&H Flanger",
			"S&H Driver", "Bass Synth (Boss SYB-3)", "Danish Driver (t.c. Line Driver)", "Large Pie (Big Muff)",
			"Pig Foot (Hog's Foot)", "Rodent (Rat)", "Bypass" };

	/**
	 * Array of integers representing the mapping of cabinet models to CC message values.
	 */
	static final int[] cabTranslate = { 11, 9, 8, 0, 1, 3, 2, 6, 7, 5, 4, 12, 13, 15, 14, 10 };

	/**
	 * Array of integers representing the mapping of effect models to CC message values.
	 */
	static final int[] effectTranslate = { 11, 9, 8, 0, 1, 3, 2, 6, 7, 5, 4, 12, 13, 15, 14, 10 };

	/**
	 * Array of strings representing the two possible locations of the volume pedal (pre-amp or post amp).
	 */
	static final String[] volPedLoc = { "Pre-Amp", "Post-Amp" };
	...
	/**
	 * A reference to the mid sweep knob widget which is hidden for some amp models.
	 */
	...
	private void addAmpPane(JPanel parentPanel, PatchDataImpl patch) {
		...
		addWidget(speakerPane, new ComboBoxWidget("Cabinet Type", patch,
				new CabTypeModel(patch, 31 + pgmDumpheaderSize), new CCCabTypeSender(71), cabType), 0, 0, 1, 1, 2);
		addWidget(speakerPane, new KnobWidget("Air", patch, 0, 127, 0, new ScaledParamModel(patch,
				32 + pgmDumpheaderSize, 127, 63), new CCSender(72)), 0, 0, 1, 1, 7);
		...
		addWidget(ampKnobsPane, new KnobWidget("Drive", patch, 0, 127, 0, new ScaledParamModel(patch,
				4 + pgmDumpheaderSize, 127, 63), new CCSender(13)), 0, 0, 1, 1, 8);

		addWidget(ampKnobsPane, new KnobWidget("Bass", patch, 0, 127, 0, new ScaledParamModel(patch,
				6 + pgmDumpheaderSize, 127, 63), new CCSender(14)), 2, 0, 1, 1, 9);
		addWidget(ampKnobsPane, new KnobWidget("Mid", patch, 0, 127, 0, new ScaledParamModel(patch,
				7 + pgmDumpheaderSize, 127, 63), new CCSender(15)), 3, 0, 1, 1, 10);

		midSweepKnob = new HideableKnobWidget("Mid Freq", patch, 0, 127, 0, new ScaledParamModel(patch,
				12 + pgmDumpheaderSize, 127, 63), new CCSender(28));
		...
		addWidget(ampKnobsPane, new KnobWidget("Treble", patch, 0, 127, 0, new ScaledParamModel(patch,
				8 + pgmDumpheaderSize, 127, 63), new CCSender(16)), 4, 0, 1, 1, 12);
		addWidget(ampKnobsPane, new KnobWidget("Volume", patch, 0, 127, 0, new ScaledParamModel(patch,
				10 + pgmDumpheaderSize, 127, 63), new CCSender(17)), 6, 0, 1, 1, 13);

		JPanel modelDescPanel = new JPanel();
		modelDesc = new JLabel();
		modelDescPanel.add(modelDesc);
		patchSelector = new ComboBoxWidget("Amp Model", patch, new AmpModelModel(patch, 3 + pgmDumpheaderSize),
	...
	private void addStompBoxPane(JPanel parentPanel, PatchDataImpl patch) {
		...
		addWidget(gatePane, new CheckBoxWidget("Noise Gate", patch, new ScaledParamModel(patch, 0 + pgmDumpheaderSize),
		...
		addWidget(gateKnobsPane, new KnobWidget("Threshold", patch, 0, 127, 0, new ScaledParamModel(patch,
				16 + pgmDumpheaderSize, 127, 96), new CCSender(23, true)), 0, 0, 1, 1, 14);
		addWidget(gateKnobsPane, new KnobWidget("Decay", patch, 0, 127, 0, new ScaledParamModel(patch,
				17 + pgmDumpheaderSize, 127, 63), new CCSender(24)), 1, 0, 1, 1, 15);
		...
		addWidget(volPedKnobsPane, new KnobWidget("Minimum", patch, 0, 127, 0, new ScaledParamModel(patch,
				23 + pgmDumpheaderSize), new CCSender(46)), 5, 0, 1, 1, 19);
		addWidget(volPedKnobsPane, new KnobWidget("Current", patch, 0, 127, 0, new ScaledParamModel(patch,
				22 + pgmDumpheaderSize), new CCSender(7)), 6, 0, 1, 1, 20);
	...
	private void addEffectsPanel(JPanel parentPanel, PatchDataImpl patch) {
		...
		addWidget(effectsPanel, new ComboBoxWidget("Effects", patch, new EffectModel(patch, 49 + pgmDumpheaderSize),
				new CCEffectSender(19), effectType), 0, 0, 1, 1, 5);

		JPanel effOnOffPanel = new JPanel();
		effectsPanel.add(effOnOffPanel);
		addWidget(effOnOffPanel, new CheckBoxWidget("Enable FX", patch, new ScaledParamModel(patch,
				52 + pgmDumpheaderSize), new CCSender(60, 127)), 0, 0, 1, 1, -3);
		addWidget(effOnOffPanel, new CheckBoxWidget("FX to D.I.", patch, new ScaledParamModel(patch,
				2 + pgmDumpheaderSize), new CCSender(64, 127)), 0, 0, 1, 1, -4);
	...
	private void setupOctaveDownPanel(JPanel parentPanel, PatchDataImpl patch) {
		...
		addWidget(parmsPanel, new KnobWidget("Mix", patch, 0, 127, 0, new ScaledParamModel(patch,
				53 + pgmDumpheaderSize, 127, 63), new CCSender(29)), 6, 0, 1, 1, 21);
	}
	...
	private void setupAnalogChorusPanel(JPanel parentPanel, PatchDataImpl patch) {
		...
		JPanel parmsPanel = new JPanel();
		parentPanel.add(parmsPanel);
		addWidget(parmsPanel, new KnobWidget("Speed", patch, 0, 127, 0, new ScaledParamModel(patch,
				53 + pgmDumpheaderSize, 127, 63), new CCSender(30)), 6, 0, 1, 1, 22);
		addWidget(parmsPanel, new KnobWidget("Depth", patch, 0, 127, 0, new ScaledParamModel(patch,
				54 + pgmDumpheaderSize, 127, 63), new CCSender(31)), 6, 0, 1, 1, 23);
	}
	...
	private void setupDanishChorusPanel(JPanel parentPanel, PatchDataImpl patch) {
		...
		JPanel parmsPanel = new JPanel();
		parentPanel.add(parmsPanel);
		addWidget(parmsPanel, new KnobWidget("Width", patch, 0, 127, 0, new ScaledParamModel(patch,
				53 + pgmDumpheaderSize, 127, 63), new CCSender(34)), 6, 0, 1, 1, 24);
		addWidget(parmsPanel, new KnobWidget("Speed", patch, 0, 127, 0, new ScaledParamModel(patch,
				54 + pgmDumpheaderSize, 127, 63), new CCSender(32)), 6, 0, 1, 1, 25);
		addWidget(parmsPanel, new KnobWidget("Intensity", patch, 0, 127, 0, new ScaledParamModel(patch,
				55 + pgmDumpheaderSize, 127, 63), new CCSender(33)), 6, 0, 1, 1, 26);
	}
	...
	private void setupOPhasePanel(JPanel parentPanel, PatchDataImpl patch) {
		...
		parentPanel.add(parmsPanel);
		addWidget(parmsPanel, new KnobWidget("Speed", patch, 0, 127, 0, new ScaledParamModel(patch,
				53 + pgmDumpheaderSize, 127, 63), new CCSender(35)), 6, 0, 1, 1, 27);
	}
	...
	private void setupGrayFlangerPanel(JPanel parentPanel, PatchDataImpl patch) {
		...
		parentPanel.add(parmsPanel);
		addWidget(parmsPanel, new KnobWidget("Width", patch, 0, 127, 0, new ScaledParamModel(patch,
				55 + pgmDumpheaderSize, 127, 63), new CCSender(36)), 6, 0, 1, 1, 28);
		addWidget(parmsPanel, new KnobWidget("Speed", patch, 0, 127, 0, new ScaledParamModel(patch,
				53 + pgmDumpheaderSize, 127, 63), new CCSender(37)), 6, 0, 1, 1, 29);
		addWidget(parmsPanel, new KnobWidget("Regen", patch, 0, 127, 0, new ScaledParamModel(patch,
				54 + pgmDumpheaderSize, 127, 63), new CCSender(38)), 6, 0, 1, 1, 30);
	}
	...
	private void setupSHFlangerPanel(JPanel parentPanel, PatchDataImpl patch) {
		...
		addWidget(parentPanel, new KnobWidget("Speed", patch, 0, 127, 0, new ScaledParamModel(patch,
				53 + pgmDumpheaderSize, 127, 63), new CCSender(41)), 6, 0, 1, 1, 35);
		addWidget(parentPanel, new KnobWidget("Fl. Speed", patch, 0, 127, 0, new ScaledParamModel(patch,
				54 + pgmDumpheaderSize, 127, 63), new CCSender(37)), 6, 0, 1, 1, 36);
		addWidget(parentPanel, new KnobWidget("Regen", patch, 0, 127, 0, new ScaledParamModel(patch,
				55 + pgmDumpheaderSize, 127, 63), new CCSender(38)), 6, 0, 1, 1, 37);
	}
	...
	private void setupSHDriverPanel(JPanel parentPanel, PatchDataImpl patch) {
		...
		addWidget(parentPanel, new KnobWidget("Distortion", patch, 0, 127, 0, new ScaledParamModel(patch,
				54 + pgmDumpheaderSize, 127, 127), new CCSender(55)), 6, 0, 1, 1, 39);
	}
	...
	private void setupBassSynthPanel(JPanel parentPanel, PatchDataImpl patch) {
		...
		addWidget(parentPanel, new KnobWidget("Dry Mix", patch, 0, 127, 0, new ScaledParamModel(patch,
				54 + pgmDumpheaderSize, 127, 63), new CCSender(48)), 6, 0, 1, 1, 41);
		addWidget(parentPanel, new KnobWidget("Lowpass", patch, 0, 127, 0, new ScaledParamModel(patch,
				55 + pgmDumpheaderSize, 127, 63), new CCSender(49)), 6, 0, 1, 1, 42);
		addWidget(parentPanel, new KnobWidget("Bandpass", patch, 0, 127, 0, new ScaledParamModel(patch,
				56 + pgmDumpheaderSize, 127, 63), new CCSender(50)), 6, 0, 1, 1, 43);
	}
	...
	private void setupRodentPanel(JPanel parentPanel, PatchDataImpl patch) {
		...
		addWidget(parentPanel, new KnobWidget("Distortion", patch, 0, 127, 0, new ScaledParamModel(patch,
				53 + pgmDumpheaderSize, 127, 127), new CCSender(58)), 6, 0, 1, 1, 48);
	}
	...
	private void addParametricPanel(JPanel parentPanel, PatchDataImpl patch) {
		...
		parmPanel.setBorder(new TitledBorder(new EtchedBorder(EtchedBorder.RAISED), "Parametric EQ",
				TitledBorder.CENTER, TitledBorder.CENTER));
		parmPanel.setLayout(new BoxLayout(parmPanel, BoxLayout.X_AXIS));
		parametricPanel.add(parmPanel);
		addWidget(parmPanel, new KnobWidget("Gain", patch, 0, 127, 0, new ScaledParamModel(patch,
				15 + pgmDumpheaderSize, 127, 63), new CCSender(27)), 6, 0, 1, 1, 49);
		addWidget(parmPanel, new KnobWidget("Q", patch, 0, 127, 0, new ScaledParamModel(patch, 14 + pgmDumpheaderSize,
				127, 63), new CCSender(26)), 6, 0, 1, 1, 50);
		addWidget(parmPanel, new KnobWidget("Freq", patch, 0, 127, 0, new ScaledParamModel(patch,
				13 + pgmDumpheaderSize, 127, 63), new CCSender(25)), 6, 0, 1, 1, 51);

		JPanel loCutPanel = new JPanel();
		loCutPanel.setBorder(new TitledBorder(new EtchedBorder(EtchedBorder.RAISED), "FX Lo-Cut Filter",
				TitledBorder.CENTER, TitledBorder.CENTER));
		loCutPanel.setLayout(new BoxLayout(loCutPanel, BoxLayout.Y_AXIS));
		parametricPanel.add(loCutPanel);
		addWidget(loCutPanel, new KnobWidget("Off   1kHz", patch, 0, 127, 0, new ScaledParamModel(patch,
				51 + pgmDumpheaderSize, 127, 63), new CCSender(21)), 6, 0, 1, 1, 52);
	}
	...
	private void addCompressPanel(JPanel parentPanel, PatchDataImpl patch) {
		...
		addWidget(btnPanel, new ComboBoxWidget("Ratio", patch, new ScaledParamModel(patch, 25 + pgmDumpheaderSize, 4,
				100), new CCSender(42, 25), new String[] { "2:1", "3.3:1(LA-2A)", "8:1", "12:1", "Inf:1" }), 0, 0, 1,
				1, 6);

		JPanel parmPanel = new JPanel();
		compressPanel.add(parmPanel);
		addWidget(parmPanel, new KnobWidget("Lev/Thresh", patch, 0, 127, 0, new ScaledParamModel(patch,
				11 + pgmDumpheaderSize, 127, 63), new CCSender(18)), 6, 0, 1, 1, 53);
		addWidget(parmPanel, new KnobWidget("Attack", patch, 0, 127, 0, new ScaledParamModel(patch,
				28 + pgmDumpheaderSize, 127, 127), new CCSender(51)), 6, 0, 1, 1, 54);
		addWidget(parmPanel, new KnobWidget("Release", patch, 0, 127, 0, new ScaledParamModel(patch,
				27 + pgmDumpheaderSize, 127, 127), new CCSender(63)), 6, 0, 1, 1, 55);
	}
	 ...
	 * the setCtrlVisibility method, which determines if the mid sweep knob should be visible or not.
	 */
	...
	/**
	 * Sets the mid sweep knob to visible or not, depending upon the amp model selected by the user.
	 */
	...
	/**
	 * CCSender class for cabinet model. Translates the value sent by the comboboxWidget to the CC numbers for the
	 * cabinet parameter.
	 */
	...
	/**
	 * ParamModel class for cabinet model. Gets and sets the cabinet model value in the sysex record. Also handles the
	...
	class CabTypeModel extends ParamModel {
		...
		/**
		 * Updates the sysex record for the cabinet model parameter. Converts the comboBoxWidget value to a valid
		 * cabinet model CC value.
		 */
		public void set(int i) {
