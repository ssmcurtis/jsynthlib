 ...
 * header and the program patch has a 9 byte header. The only reason for having this driver is to be able to request an
 * edit buffer patch. As soon as the edit buffer patch is received, Line6BassPodConverter converts it to a regular
 * program patch. From that point on it is handled like any other program patch.
 * 
 * @author Jeff Weber
 */
public class Line6BassPodEdBufDriver extends Line6BassPodSingleDriver {
	 ...
	 */
	private static final SysexHandler SYS_REQ = new SysexHandler(Constants.EDIT_DUMP_REQ_ID); // Edit Buffer Dump
																								// Request
