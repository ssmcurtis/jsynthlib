 ...
 * adjustment and the other for fine adjustment. The range for coarse sysex values is 0-97536, where the max value is
 * represented in bytes 27-29 as 0x017D00. The range for fine sysex values is 0-762, where the max value is represented
 * in bytes 27-29 as 0x0002FA. The value stored in offset 27-29 of the sysex record is simply the sum of the course and
 * fine values. So the value with the coarse and fine values both set to maximum is 98298 or 0x017FFA. For the coarse
 * control the control range of 0-127 split over the range 0-97536 gives steps of 768 or 0x300. The two values are
...
class DelayCoarseSpeedModel extends ParamModel {
	/**
	 * The minimum CC value that will be used for the control. This is used when calculating a scaling factor. The
	 * scaling factor is calculated as (maxSysex - minSysex) / (maxCC - minCC).
	 */
	private int minCC = 0;

	/**
	 * The maximum CC value that will be used for the control. This is used when calculating a scaling factor. The
	 * scaling factor is calculated as (maxSysex - minSysex) / (maxCC - minCC).
	 */
	private int maxCC;
	...
	/**
	 * The scaling factor, calculated as (maxSysex - minSysex) / (maxCC - minCC).
	 */
	private int scaleFactor;

	/**
	 * Constructs a DelayCoarseSpeedModel. Patch p is the reference to the patch containing the sysex record. int o is
	...
	/**
	 * Constructs a DelayCoarseSpeedModel. Patch p is the reference to the patch containing the sysex record. int o is
	 * the offset into the sysex record in non-nibblized bytes, not including the header bytes. int min CC and int maxCC
	...
	/**
	 * Scales the value of i according scaleFactor and sets the sysex value at offset ofs to the result.
	 */
	...
	/**
	 * Returns the value of the two bytes representing the delay speed as an int. The value returned is the composite
	 * value representing both the coarse speed and the fine speed.
	 */
	private int getBytesValue() {
		int msB = (int) PatchBytes.getSysexByte(patch.getSysex(), 9, ofs);
		int lsB = (int) PatchBytes.getSysexByte(patch.getSysex(), 9, ofs + 1);
