 ...
 * patch locations apiece. The banks are numbered 1 through 9 and the patches are designated by the letters A, B, C, and
 * D. The actual physical layout is a single bank consisting of 36 patch locations numbered 0 to 35.
 * 
 * @author Jeff Weber
 */
public class Line6Pod20BankDriver extends SynthDriverBank {
	/**
	 * Program Bank Dump Request
	 */
	private static final SysexHandler SYS_REQ = new SysexHandler(Constants.BANK_DUMP_REQ_ID); // Program Bank Dump
																								// Request
	...
	/**
	 * Returns the offset within a virtual (un-nibblized) bank patch of the single patch given by patchNum. The value
	 * returned includes the header bytes and is represents the offset in un-nibblized (non-native) bytes. Called by
	 * getPatchName and setPatchName.
	 */
	...
	public PatchDataImpl getPatch(PatchDataImpl bank, int patchNum) {
		...
		try {
			...
			ErrorMsgUtil.reportError("Error", "Error in Pod 2.0 Bank Driver", e);
			return null;
		}
	}

	/**
	 * Returns the offset within a native (nibblized) bank patch of the single patch given by patchNum. The value
	 * returned includes the header bytes and represents the offset in nibblized (Line6-native) bytes. Called by
