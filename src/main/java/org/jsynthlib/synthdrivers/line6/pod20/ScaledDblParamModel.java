...
class ScaledDblParamModel extends ParamModel {
	/**
	 * The mminimum CC value that will be used for the control. This is used when calculating a scaling factor. The
	...
	/**
	 * Reverses the range of values sent by the sysexWidget over the range max to min. A value of true causes the range
	 * to be reversed.
	 */
	...
	/**
	 * Constructs a ScaledDblParamModel. This is used to scale values when the max sysex value is different than the max
	 * CC value. Patch p is the reference to the patch containing the sysex record. int o is the offset into the sysex
	...
	/**
	 * Constructs a ScaledDblParamModel. This is used to scale values when the max sysex value is different than the max
	 * CC value and the minimum CC and sysex values are not zero. Patch p is the reference to the patch containing the
	 * sysex record. int o is the offset into the sysex record in non-nibblized bytes, not including the header bytes.
	 * int minCC and int maxCC are the the minimum and maximum CC values, respectively. int minSysex and int maxSysex
	...
	/**
	 * Scales the value of i according maxSysex and sets the sysex value at offset ofs to the result. Also reverses the
	 * value over the range if reverse is true.
	 */
	...
	/**
	 * Returns the value at offset ofs, scaled according to maxCC and maxSysex. Also reverses the value over the range
	 * if reverse is true.
	 */
	public int get() {
