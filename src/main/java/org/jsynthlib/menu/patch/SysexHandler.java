 ...
 * <LI>On converstion to byte array (prior to being sent as a sysex message): <DIR>
 ...
 * <LI><code>##</code> - Replaced by the value of argument <code>deviceID</code> argument + 16.
 ...
 * send(sysexHandler.toSysexMessage(getDeviceID(), new SysexHander.NameValue(&quot;patchNum&quot;, patchNum),
		...
		for (int iByte = 0; st.hasMoreTokens(); iByte++) {
			...
			default:
				try {
					...
					ErrorMsg.reportError("SysexHandler", "Invalid number, " + sToken
			...
			default:
				if (src.charAt(iSrc + 1) == '*') { // eg 0*
					...
					try {
						...
						ErrorMsg.reportError("SysexHandler", "Invalid number, " + src.substring(iSrc, iSrc + 2)
