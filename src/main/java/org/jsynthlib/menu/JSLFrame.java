...
/**
 * A Frame class which supports JInternalFrame methods and also can creates a non-internal frame using JFrame. For the
 * details of each method, refer the documentation of JInternalFrame.
 * 
 * @see JSLDesktop
 * @see JSLWindowMenu
...
public class JSLFrame {
	...
	/**
	 * Creates a non-resizable, non-closable, non-maximizable, non-iconifiable JSLFrame with no title.
	 */
	...
	/**
	 * Creates a JSLFrame with the specified title, resizability, closability, maximizability, and iconifiability.
	 */
	public JSLFrame(JSLDesktop desktop, String title, boolean resizable, boolean closable, boolean maximizable,
			boolean iconifiable) {
		this.desktop = desktop;
		if (JSLDesktop.useMDI())
			proxy = new JSLIFrame(this, title, resizable, closable, maximizable, iconifiable);
		else
	...
	/**
	 * Adds the specified focus listener to receive focus events from this component when this component gains input
	 * focus.
	 */
	...
	class JSLIFrame extends JInternalFrame implements JSLFrameProxy, InternalFrameListener {
		...
		public JSLIFrame(JSLFrame p, String title, boolean resizable, boolean closable, boolean maximizable,
				boolean iconifiable) {
		...
		public void setVisible(boolean b) {
			ErrorMsgUtil.reportStatus(ErrorMsgUtil.FRAME, "setVisible : " + this.getTitle());
			super.setVisible(b);
			try {
				this.setSelected(b);
		...
		public void internalFrameActivated(InternalFrameEvent e) {
			ErrorMsgUtil.reportStatus(ErrorMsgUtil.FRAME, "\"" + this.getTitle() + "\" activated.");
			JSLFrameEvent fe = new JSLFrameEvent(getJSLFrame(), JSLFrameEvent.ACTIVATED);
		...
		public void internalFrameClosed(InternalFrameEvent e) {
			JSLFrameEvent fe = new JSLFrameEvent(getJSLFrame(), JSLFrameEvent.CLOSED);
			Iterator it = listeners.iterator();
			while (it.hasNext()) {
		...
		public void internalFrameClosing(InternalFrameEvent e) {
			JSLFrameEvent fe = new JSLFrameEvent(getJSLFrame(), JSLFrameEvent.CLOSING);
			Iterator it = listeners.iterator();
			while (it.hasNext()) {
		...
		public void internalFrameDeactivated(InternalFrameEvent e) {
			ErrorMsgUtil.reportStatus(ErrorMsgUtil.FRAME, "\"" + this.getTitle() + "\" deactivated.");
			JSLFrameEvent fe = new JSLFrameEvent(getJSLFrame(), JSLFrameEvent.DEACTIVATED);
			Iterator it = listeners.iterator();
			while (it.hasNext()) {
		...
		public void internalFrameDeiconified(InternalFrameEvent e) {
			JSLFrameEvent fe = new JSLFrameEvent(getJSLFrame(), JSLFrameEvent.DEICONIFIED);
			Iterator it = listeners.iterator();
			while (it.hasNext()) {
		...
		public void internalFrameIconified(InternalFrameEvent e) {
			JSLFrameEvent fe = new JSLFrameEvent(getJSLFrame(), JSLFrameEvent.ICONIFIED);
			Iterator it = listeners.iterator();
			while (it.hasNext()) {
		...
		public void internalFrameOpened(InternalFrameEvent e) {
			JSLFrameEvent fe = new JSLFrameEvent(getJSLFrame(), JSLFrameEvent.OPENED);
			Iterator it = listeners.iterator();
			while (it.hasNext()) {
	...
	class JSLJFrame extends JFrame implements JSLFrameProxy, WindowListener {
		...
		private void showState(String s) {
			ErrorMsgUtil.reportStatus(ErrorMsgUtil.FRAME, "\"" + this.getTitle() + "\" " + s + " (" + this.getExtendedState()
					+ ")");
		}
		...
		void fakeActivate() {
			WindowEvent we = new WindowEvent(this, WindowEvent.WINDOW_ACTIVATED, null);
