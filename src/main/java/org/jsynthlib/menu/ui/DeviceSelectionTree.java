...
public class DeviceSelectionTree extends JTree {
	...
	/**
	 * This method gets a list of all known DeviceDescriptors and then passes them to either buildTreeNoGrouping(),
	 * buildTreeByManufacturer(), or buildTreeByType(), depending upon the groupStyle.
	 * 
	 * @param node
	...
	/**
	 * This method takes a collection of DeviceDescriptors and makes sub-nodes for all manufacturers of the devices in
	 * the collection. If groupStyle is GROUP_MANUF_TYPE, then it has buildTreeByType sub-group the devices even further
	 * by type.
	 * 
	 * @param col
	 *            The collection of DeviceDescriptors to add
	...
	/**
	 * This method takes a collection of DeviceDescriptors and makes sub-nodes for all types of the devices in the
	 * collection. If groupStyle is GROUP_TYPE_MANUF, then it has buildTreeByManufacturer sub-group the devices even
	 * further by manufacturer.
	 * 
	 * @param col
	 *            The collection of DeviceDescriptors to add
	 * @param node
	...
	private void buildTreeByType(Collection col, DefaultMutableTreeNode node) {
		...
		for (Iterator i = getTypesInCollection(col).iterator(); i.hasNext();) {
			...
			// step in the grouping. Just add the whole sub-collection to this node. Otherwise, if we're
			// grouping by Type,Manufacturer, then we need to group this sub-collection by manufacturer. Emenaker
			// 2005-03-04
			switch (groupStyle) {
			case GROUP_TYPE_MANUF:
	...
	/**
	 * This class overrides a normal DefaultTreeSelectionModel used by JTrees. This is to prevent the user from being
	 * able to select any node that isn't a *leaf* (ie, an actual devicename). This way, the user can't select
	...
	class DeviceSelectionModel extends DefaultTreeSelectionModel {
		...
		/**
		 * This overrides the superclass' setSelectionPath. We need to figure out if the user clicked on a leaf (ie, an
		 * actual drivernname) or a non-leaf (ie, a type or manufacturer) We don't want to let them select non-leafs -
		 * Emenaker 2005-03-03
		 */
