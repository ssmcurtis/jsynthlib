...
public class BankPrinter implements Printable {
	...
	public int print(Graphics g, PageFormat pageFormat, int pageIndex) throws PrinterException {
		...
		/*
		 * if (pageIndex + 1 == totalNumPages) { int lastRowPrinted = numRowsOnAPage * pageIndex; int numRowsLeft =
		 * table.getRowCount() - lastRowPrinted; g2.setClip(0, (int) (tableBodyHeightOnPage * pageIndex), (int)
		 * Math.ceil(tableWidthOnPage), (int) Math.ceil(oneRowHeight * numRowsLeft)); } else { //else clip to the entire
		 * area available. g2.setClip(0, (int) (tableBodyHeightOnPage * pageIndex), (int) Math.ceil(tableWidthOnPage),
		...
		/*
		 * For debugging. This draws lines on the page where the table should go... // Draw lines over the row
		 * separators.... for(int i=0; i<=numRowsOnThisPage; i++) { g2.drawLine(0,(int) ( oneRowHeight * (double) i),
		 * (int) tableWidthOnPage, (int) ( oneRowHeight * (double) i)); }
		 */
		g2.setTransform(unscaledTransform);
		...
		g2.setClip(0, 0, (int) Math.ceil(tableWidthOnPage), (int) Math.ceil(tableHeaderHeightOnPage));
