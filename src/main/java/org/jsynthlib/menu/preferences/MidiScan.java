...
/**
 * Detect MIDI devices by sending out Inquery ID Sysex Message to every MIDI outport port with every DeviceID and poll
...
public class MidiScan extends Thread {
	...
	public void run() {
		...
		byte[] idData = { (byte) 0xf0, (byte) 0x7e, (byte) 0, (byte) 0x06, (byte) 0x01, (byte) 0xf7 };
		...
		for (int j = 0; j < maxout; j++) { // For all Outputs
			...
			for (int devID = 0; devID <= MAX_DEVICE_ID; devID++) { // every devID
				...
				try {
					MidiUtil.send(rcvr, inqMsg, AppConfig.getMidiOutBufSize(), AppConfig.getMidiOutDelay());
				} catch (MidiUnavailableException e) {
					ErrorMsg.reportStatus(e);
					continue;
				} catch (InvalidMidiDataException e) {
				...
				for (int i = 0; i < maxin; i++) { // For all Inputs
					...
					if (((answerData[0] & 0xff) == 0xf0) && (answerData[1] == 0x7e)
	...
	private void checkResponseData(byte[] answerData, int msgsize, int midiout, int midiin, int devID) {
		...
		if (!found) {
			...
			report += "Unknown Synthesizer found:\n" + " According to the manufacturer ID it is made by "
					+ ManufacturerLookup.get(answerData[5], answerData[6], answerData[7])
					+ "\n It is connected on the Output Port: " + MidiUtil.getOutputMidiDeviceInfo(midiout).toString()
					+ " with Device ID: " + devID + 1 + "\n The Input Port was: "
