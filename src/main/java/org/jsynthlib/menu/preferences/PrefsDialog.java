...
public class PrefsDialog extends JDialog {
	...
	/**
	 * Creates user preference setting dialog window. Allows for different buttons to be enabled on the dialog box
	 * 
	 * @param parent
	 *            Parent frame
	 * @param buttonsAllowed
	 *            Either {@link #OK OK}, {@link #APPLY APPLY}, and/or {@link #CANCEL CANCEL}.
	 */
	...
	/**
	 * Constructor which both {@link #OK OK}, {@link #APPLY APPLY}, and {@link #CANCEL CANCEL} button are enabled.
	 * 
	 * @param parent
	 *            the parent JFrame
	 */
	 ...
	 * 
	 * It is possible that some of the panels that we were using have, since, been used in other containers (like a
	 * stand-alone frame for one of the panels). If this happened, then the Java Swing/AWT system automatically took
	 * that panel OUT of our container (because a component can only have one parent container). To "reclaim" those
	 * panels, we need to start over with a new panel and add the config panels.
	 */
	// emenaker 2003.03.17
	private void init() {
		...
		default:
			...
			/*
			 * jtp.addChangeListener(new ChangeListener() { public void stateChanged(ChangeEvent e) {
	...
	/**
	 * This adds a tab to the configuration dialog box. Actually, it merely adds it to the vector which holds all of the
	 * panels. The actual tabbed panes won't be created unless you call this.init() - emenaker 2003.03.12
	 * 
	 * @param panel
	...
	/**
	 * ConfigPanel have to call this method when a parameter is changed or saved.
	 */
	...
	// emenaker 2003.03.12
	/*
	 * private void commitChanges() { // This used to be OKPressed, but I changed it to allow for // the "Apply" button
	 * - emenaker 2003.03.12 Enumeration e = configpanels.elements(); while (e.hasMoreElements()) { ((ConfigPanel)
	 * e.nextElement()).commitSettings(); } }
	 */
	...
	/**
	 * All this does is hide the dialog box. It was moved out of commitChanges (which used to be called OKPressed) so
	 * that the "Apply" button can save changes without exiting and the "Cancel" button can exit without saving changes.
	 * - emenaker 2003.03.14
	 */
	private void exit() {
