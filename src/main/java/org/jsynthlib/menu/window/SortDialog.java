...
public class SortDialog extends JDialog {
		...
		try {
			...
			JButton done = new JButton(" OK ");
				...
				public void actionPerformed(ActionEvent e) {
					...
					((LibraryFrame) PatchBayApplication.getDesktop().getSelectedFrame()).sortPatch(new MyOrder(command));
	...
	private static class MyOrder implements Comparator {
		...
		public int compare(Object a1, Object a2) {
			...
			String s2;
			if (field == 0) {
				s1 = ((Patch) a1).getName().toLowerCase();
				s2 = ((Patch) a2).getName().toLowerCase();
			} else if (field == 1) {
