...
public class DocumentationWindow extends JSLDialog {
	...
	public DocumentationWindow(String contentType, URL url) {
				...
				if (e.getEventType() == HyperlinkEvent.EventType.ACTIVATED) {
					if ((e.getDescription()).startsWith("#")) {
						...
						try {
							...
							ErrorMsgUtil.reportError("Error", e.getDescription().toString(), e2);
						}
					} else {
						// Link to other file
						try {
							...
							ErrorMsgUtil.reportError("Error", e.getURL().toString(), e3);
						}
					}
				}
			}
		...
		try {
			...
			ErrorMsgUtil.reportError("Error", "Wrong URL", e);
		} catch (java.io.IOException e) {
			ErrorMsgUtil.reportError("Error", "Error opening documentation", e);
		}
