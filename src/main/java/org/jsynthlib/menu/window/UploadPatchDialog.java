...
public class UploadPatchDialog extends JDialog {
	...
	public UploadPatchDialog(JFrame Parent) {
		...
		String patchType;
		PatchBasket library = (PatchBasket) PatchBayApplication.getDesktop().getSelectedFrame();
	...
	void uploadPatch(Patch p) {
		...
		byte[] sysex;
		{
			...
			if (msgs == null)
				ErrorMsgUtil.reportError("Error", "Patch contains no data");
			int length = 0;
		...
		if (patchName.length() < 4)
			ErrorMsgUtil.reportError("Error", "Patch Name must be at least 4 characters.");
		else if (desc.length() < 20)
			ErrorMsgUtil.reportError("Error", "Description must be at least 20 characters.");
		else if (userName.length() < 1)
			ErrorMsgUtil.reportError("Error", "Please supply a Contributer's User Name");
		else if (passwd.length() < 1)
			ErrorMsgUtil.reportError("Error", "Please supply the Contributer's Password for the Repository");
		else if (repository.length() < 5)
			ErrorMsgUtil.reportError("Error", "Please supply the Repository URL Address");
		else {
			...
			try {
				...
				ErrorMsgUtil.reportStatus("UploadPatchDialog encoding failed.");
			}
			mime += mimeBoundary();
		...
		try {
			...
			while ((str = in.readLine()) != null) {
				...
				if (str.indexOf("<h2") > -1) {
					...
					if (st < 0 || en < 0)
						ErrorMsgUtil.reportError("Error", "Can not Understand Server Response: " + str);
					else {
						if (str.indexOf("Success") > -1) {
							ErrorMsgUtil.reportError("Info", "Patch Uploaded Successfully");
							return 1;
						} else
							ErrorMsgUtil.reportError("Error", str.substring(st + 1, en));
					}
				}
			}
			in.close();
