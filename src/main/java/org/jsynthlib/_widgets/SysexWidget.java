 ...
 * <p>
 * 
 * They can be used as is, and can be used by being extended. Here is an example of extended {@link ScrollBarWidget}
 * class.
 * 
 * <pre>
 * class MyScrollBarWidget extends ScrollBarWidget {
 * 	MyScrollBarWidget(String label, IPatch patch, int min, int max, int base, IParamModel pmodel, ISender sender) {
...
public abstract class SysexWidget extends JPanel {
	...
	/**
	 * layout Widgets. By overriding this method, a patch editor can change the layout as he wants.
	 */
	...
	/**
	 * Set value, and update widget state. An extended class has to override this method to let update the widget state
	 * (i.e. move a slider to the value set, etc.).
	 */
	...
	/**
	 * Set value by using <code>paramModel.get</code>, and update the widget state.
	 */
	...
	/**
	 * Set value and send MIDI messages for the value to the MIDI port of <code>driver</code>. An extended class calls
	 * this when widget state is chagned.
	 * <p>
	 ...
	 * this when widget state is chagned.
	 * <p>
	 * This method does not update the widget state nor do min/max range check. It is caller's responsibility to do
	 * that.
	 */
	...
	/**
	 * Setter of label. This does not change the text in <code>jlabel</code> widget.
	 */
	...
	/*
	 * Getter of <code>paramModel</code>. public ParamModel getModel() { return paramModel; }
	 */
	/**
	...
	public void setSliderNum(int num) {
		_setSliderNum(num);
		if (num > 0)
			setToolTipText("Fader Bank " + ((num - 1) / 16 + 1) + "  Slider " + (((num - 1) % 16) + 1));
		if (num < 0) {
			num = 0 - num;
			setToolTipText("Fader Bank " + ((num - 1) / 16 + 1) + "  Button " + (((num - 1) % 16) + 1));
		}
	}
	...
	/**
	 * Set the value specified by <code>fader</code> and send System Exclusive message to a MIDI port.
	 * <p>
	 ...
	 * 
	 * Parameter Model keeps track of the changes to the patch so that when we next call up this patch the changes are
	 * there. This is also used to set the widgets to the correct values for a particular patch when the Single Editor
	 * is opened.
	 * 
	 * @see SysexWidget
	 * @see ParamModel
